...
if (Test-Path $startScript) {
    ...
    Write-Host "Finance-Microservice-Startskript wurde aufgerufen: $startScript" -ForegroundColor Green
} else {
...
try {
    ...
    if ($response.StatusCode -eq 200) {
        Write-Host "Finance-Microservice wurde erfolgreich neu gestartet und antwortet auf Health-Checks" -ForegroundColor Green
        exit 0
    } else {
        Write-Host "Finance-Microservice wurde gestartet, antwortet aber nicht korrekt (Status: $($response.StatusCode))" -ForegroundColor Yellow
        exit 1
    }
} catch {
    Write-Host "Finance-Microservice reagiert noch nicht auf Health-Checks, möglicherweise ist der Start noch nicht abgeschlossen" -ForegroundColor Yellow
    exit 1
} 