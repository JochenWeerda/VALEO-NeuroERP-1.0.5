...
from backend.app.ai.openrouter_client import horizon_beta_assistant, OpenRouterMessage
...
class AIRequest(BaseModel):
    ...
    use_horizon_beta: bool = True  # Standardmäßig Horizon Beta verwenden
...
class VALEOAssistant:
    ...
    async def process_request(self, request: AIRequest) -> AIResponse:
        ...
        try:
            ...
            metrics_collector.record_business_event("ai_request", {
                ...
                "use_horizon_beta": request.use_horizon_beta
            })
            
            # Use Horizon Beta if requested
            if request.use_horizon_beta:
                return await self._process_with_horizon_beta(request)
    ...
    async def _process_with_horizon_beta(self, request: AIRequest) -> AIResponse:
        """Process request using Horizon Beta"""
        
        try:
            # Determine context for Horizon Beta
            context_map = {
                AssistantCapability.PERSONAL_MANAGEMENT: "valeo_general",
                AssistantCapability.LAGERVERWALTUNG: "valeo_warehouse",
                AssistantCapability.CRM: "valeo_crm",
                AssistantCapability.REPORTING: "valeo_finance",
                AssistantCapability.FINANZBUCHHALTUNG: "valeo_finance",
                # Add more mappings as needed
            }
            
            horizon_context = context_map.get(request.context.module, "valeo_general")
            
            # Get response from Horizon Beta
            horizon_response = await horizon_beta_assistant.get_response(
                user_query=request.query,
                context=horizon_context,
            ...
            return AIResponse(
                answer=horizon_response,
                suggestions=suggestions,
                confidence=0.85,
                model_used="horizon-beta",
                metadata={
            ...
            logger.error(f"Horizon Beta processing failed: {str(e)}")
