...
def upgrade() -> None:
    ...
    op.execute("""
        ...
        WHERE r.name = 'manager' AND p.name IN (
            ...
            'invoice.approve', 'order.view', 'order.create',
            'order.update', 'order.approve', 'stock.view',
        ...
        WHERE r.name = 'accountant' AND p.name IN (
            ...
            'invoice.update', 'report.view', 'report.export',
            'report.financial'
        );
        ...
        WHERE r.name = 'warehouse' AND p.name IN (
            'article.view', 'article.update', 'stock.view',
            'stock.update', 'stock.inventory', 'stock.movement'
        );
        ...
        WHERE r.name = 'sales' AND p.name IN (
            'customer.view', 'customer.create', 'customer.update',
            'invoice.view', 'invoice.create', 'order.view',
            'order.create'
        );
        ...
        WHERE r.name = 'viewer' AND p.name IN (
            'article.view', 'customer.view', 'invoice.view',
            'order.view', 'stock.view', 'report.view'
        );
    """)
    ...
    op.execute("""
        ...
        SELECT DISTINCT
            ...
            COALESCE(up.granted_at, ur.assigned_at) as granted_at,
            up.expires_at
        FROM users u
...
def downgrade() -> None:
    """Remove RBAC tables and data"""
    
    # Drop views and functions
    op.execute("DROP VIEW IF EXISTS user_permissions_view;")
    op.execute("DROP FUNCTION IF EXISTS user_has_permission(INTEGER, TEXT);")
