"""
Vereinfachtes Starter-Skript für den Observer-Service (Python 3.11 kompatibel)
"""
...
from observer_service import ObserverService, register_service_api
try:
    from simple_optimizer import SimplePerformanceOptimizer
except ImportError:
    SimplePerformanceOptimizer = None
    print("WARNUNG: SimplePerformanceOptimizer konnte nicht importiert werden.")
...
class ObserverStarter:
    ...
    def parse_arguments(self):
        parser = argparse.ArgumentParser(description="Startet den Observer-Service")
    ...
    def start_observer(self):
        ...
        try:
            ...
            # Starten des Observers in einem separaten Thread
            observer_thread = threading.Thread(target=self.observer.start)
            observer_thread.daemon = True
            observer_thread.start()
            logger.info("Observer gestartet in separatem Thread")
            return True
    ...
    def start_optimizer(self):
        ...
        if self.args.no_optimizer or SimplePerformanceOptimizer is None:
    ...
    def export_metrics(self):
        ...
        try:
            metrics = {"metrics_history": self.observer.metrics_history}
    ...
    def start(self):
        ...
        try:
            ...
            routes = [
                ...
                Route("/register", register_service_api, methods=["POST"]),
            ]
            
            app = Starlette(routes=routes)
            # Observer-Referenz zur App hinzufügen
            app.state.observer = self.observer
            
            logger.info(f"Observer-Dashboard wird gestartet auf http://localhost:{self.args.port}")
            logger.info(f"Microservice-Registrierung verfügbar unter: http://localhost:{self.args.port}/register")
...
if __name__ == "__main__":
    print("Observer-Service für Python 3.11 wird gestartet...")
