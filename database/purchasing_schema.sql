...
CREATE TABLE einkauf.bestellungen (
    ...
    status VARCHAR(20) DEFAULT 'ERSTELLT' CHECK (status IN ('ERSTELLT', 'BESTÄTIGT', 'TEILLIEFERUNG', 'VOLLSTÄNDIG', 'STORNIERT')),
...
BEGIN
    RETURN COALESCE((
        ...
        AND b.status IN ('ERSTELLT', 'BESTÄTIGT', 'TEILLIEFERUNG')
    ), 0);
END;
$$ LANGUAGE plpgsql;
...
('LIE001', 'Agrarhandel Müller GmbH', 'Hans Müller', '+49 123 456789', 'h.mueller@agrar-mueller.de', 'DÜNGER', 5),
...
((SELECT lieferant_id FROM einkauf.lieferanten WHERE lieferant_nr = 'LIE001'), 'HAUPTADRESSE', 'Hauptstraße', '123', '12345', 'Musterstadt', TRUE),
...
((SELECT lieferant_id FROM einkauf.lieferanten WHERE lieferant_nr = 'LIE004'), 'HAUPTADRESSE', 'Maschinenstraße', '89', '45678', 'Landstadt', TRUE),
...
((SELECT lieferant_id FROM einkauf.lieferanten WHERE lieferant_nr = 'LIE004'), 'MAS001', 'Traktor Ersatzteile', 'Stück', 1, 14),
...
('BES000002', (SELECT lieferant_id FROM einkauf.lieferanten WHERE lieferant_nr = 'LIE002'), CURRENT_DATE - INTERVAL '5 days', CURRENT_DATE + INTERVAL '2 days', 'BESTÄTIGT', 4500.00),
('BES000003', (SELECT lieferant_id FROM einkauf.lieferanten WHERE lieferant_nr = 'LIE003'), CURRENT_DATE - INTERVAL '10 days', CURRENT_DATE - INTERVAL '3 days', 'VOLLSTÄNDIG', 1550.00);
...
('REK000001', (SELECT lieferant_id FROM einkauf.lieferanten WHERE lieferant_nr = 'LIE001'), CURRENT_DATE - INTERVAL '30 days', CURRENT_DATE - INTERVAL '10 days', 8500.00, 'VOLLSTÄNDIG'),
