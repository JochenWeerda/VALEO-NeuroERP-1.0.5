...
begin
  ...
  begin
    ...
    BackendPath := WelcomePage.Values[0];
    FrontendPath := WelcomePage.Values[1];
  end
  else if CurPageID = wpSelectDir then
  begin
    // Jetzt ist {app} initialisiert!
    SetIniString('Paths', 'BackendPath', BackendPath, ExpandConstant('{app}\\config.ini'));
    SetIniString('Paths', 'FrontendPath', FrontendPath, ExpandConstant('{app}\\config.ini'));
  end;
end;