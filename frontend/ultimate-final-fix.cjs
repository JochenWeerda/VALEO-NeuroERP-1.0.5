...
function fixFile(filePath) {
  try {
    ...
    const fixes = [
      ...
      {
        pattern: /const\s+(fireEvent|waitFor|user|zvooveApiService|useZvooveStore|NavigationTab|initialTab|config|record|index|key|threat|action|segment|resolve|module|form|invoice|bestellung|col|column|list|acc|img|state|formConfig|kontakt|e|row|text|deadline|callback|item|value|data|error|result|response|params|args|options|settings|config|props|children|event|handler|callback|fn|func|method|prop|attr|id|name|type|class|style|ref|key|index|idx|i|j|k|l|m|n|o|p|q|r|s|t|u|v|w|x|y|z)\s*=/g,
