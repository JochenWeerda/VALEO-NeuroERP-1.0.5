...
class FrontendErrorFixer {
  ...
  async fixFrontendErrors() {
    ...
    console.log(`📊 Found ESLint issues, analyzing...\n`);
  ...
  parseESLintOutput() {
    ...
    for (const line of lines) {
      // Datei-Zeile: c:\Users\...\file.tsx
      if (line.startsWith('c:\\') && !line.includes('  ')) {
        currentFile = line;
        continue;
      }

      // Fehler/Warning-Zeile:   line:col   severity  message
      if (currentFile && line.match(/^\s+\d+:\d+\s+(error|warning)/)) {
        const errorMatch = line.match(/^\s+(\d+):(\d+)\s+(error|warning)\s+(.+)$/);

        if (errorMatch) {
          const [, lineNum, col, severity, message] = errorMatch;
