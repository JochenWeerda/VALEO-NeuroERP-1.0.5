 ...
 */;
const compareStyles = (actual: unknown, expected: unknown, tolerance = 2) => {;
const differences: string[] = [];
  
  Object.keys(expected).forEach(key => {;
const actualValue = actual[key];, const expectedValue = expected[key];, // Prüfe ob Werte exakt übereinstimmen, if (actualValue === expectedValue) {
...
const equivalentGroups = [,
  ...
  return equivalentGroups.some(group =>, group.includes(actual) && group.includes(expected),
  );,
};;
...
const { _container,} = renderWithProviders(<SapFioriDashboard />);
...
const { _container,} = renderWithProviders(<AnalyticsPage />);
...
const { _container,} = renderWithProviders(<TrustAwareDashboard />);
