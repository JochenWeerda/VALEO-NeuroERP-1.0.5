...
export const useCustomers = (filter?: CustomerFilter) => {
  return useQuery<Customer[], Error>({
    queryKey: ['customers', filter], queryFn: () => crmService.getCustomers(filter),
  });
};

export const useCustomer = (id: string) => {
  return useQuery<Customer | null, Error>({
    queryKey: ['customer', id], queryFn: () => crmService.getCustomerById(id),
    enabled: !!id,
  });
};
...
const queryClient = useQueryClient();,
  ...
  return useMutation<Customer, Error, { id: string; data: Partial<Customer> }>({
    mutationFn: ({ _id, data }: { id: string; data: Partial<Customer> }) =>
...
export const useContactPersons = (customerId: string, filter?: ContactPersonFilter) => {
  return useQuery<ContactPerson[], Error>({
    queryKey: ['contactPersons', customerId, filter], queryFn: () => crmService.getContactPersons(customerId, filter),
    enabled: !!customerId,
  });
};
...
export const useContactPerson = (id: string) => {
  return useQuery<ContactPerson | null, Error>({
    queryKey: ['contactPerson', id], queryFn: () => crmService.getContactPersonById(id),
    enabled: !!id,
  });
};
...
const queryClient = useQueryClient();,
  ...
  return useMutation<ContactPerson, Error, ContactPersonFormData>({
    mutationFn: (formData: _ContactPersonFormData) => crmService.createContactPerson(formData),
    onSuccess: () => {
...
const queryClient = useQueryClient();,
  ...
  return useMutation<ContactPerson, Error, { id: string; formData: ContactPersonFormData }>({
    mutationFn: ({ _id, formData }: { id: string; formData: ContactPersonFormData }) =>
...
const queryClient = useQueryClient();,
  ...
  return useMutation<void, Error, string>({
    mutationFn: (id: _string) => crmService.deleteContactPerson(id),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['contactPersons'] });
    },
  });
