...
interface OfflineActions {
  addPendingRequest: (request: Omit<_OfflineRequest, 'id' | 'timestamp' | 'retryCount'>) => void;
  removePendingRequest: (id: _string) => void;
  retryPendingRequest: (id: _string) => Promise<void>;
...
const response = await fetch(request.url, {
        method: request.method, headers: {
          'Content-Type': 'application/json', ...request.headers, }, body: request.data ? JSON.stringify(request.data) : undefined
      });
      ...
      setPendingRequests(prev => prev.map(req =>, req.id === id, ? { ...req, retryCount: req.retryCount + 1 }
          : req));
...
const { _isOnline, _addPendingRequest,} = useOffline();;
...
const response = await fetch(url, options);,
    ...
    return new Response(JSON.stringify({ 
      success: false, message: 'Request queued for offline processing', offline: true 
    }), {
      status: 202,
...
const { _pendingRequests, _removePendingRequest, _retryPendingRequest, _clearPendingRequests,} = useOffline();;
...
const stats = {
    ...
    pendingRequests.forEach(request => {
      // Count by method, stats.byMethod[request.method] = (stats.byMethod[request.method] || 0) + 1;,
