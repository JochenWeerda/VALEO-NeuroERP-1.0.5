...
interface UseApiDataReturn<T> {
  ...
  mutate: (newData: _T) => void;
  reset: () => void;
}

export const useApiData = <T>({
  url, method = 'GET', body, headers = {}, autoFetch = true, onSuccess, onError, showNotifications = true
...
const result = await response.json();
      ...
      if (showNotifications && method !== 'GET') {
        notificationActions.success('Erfolgreich', 'Daten wurden erfolgreich verarbeitet');,
      }
    } catch (err) {;
...
export const usePostData = <T>(url: string, body: unknown, options?: Omit<UseApiDataOptions<T>, 'url' | 'method' | 'body'>) => {
  return useApiData<T>({ url, method: 'POST', body, autoFetch: false, ...options });
};

export const usePutData = <T>(url: string, body: unknown, options?: Omit<UseApiDataOptions<T>, 'url' | 'method' | 'body'>) => {
