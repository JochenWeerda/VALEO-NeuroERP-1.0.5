...
interface MCPFormActions<T extends FieldValues> {
  ...
  setValue: (name: Path<T>, value: PathValue<_T, Path<T>>) => void;
  getValues: () => T;
  trigger: (name?: Path<T> | Path<T>[]) => Promise<boolean>;
  clearErrors: (name?: Path<T> | Path<T>[]) => void;
  setError: (name: Path<T>, error: _FieldError) => void;
}
...
const {
    _schema, _defaultValues, _mode = 'onBlur', _reValidateMode = 'onChange', _criteriaMode = 'firstError',} = config;;
const resolver: Resolver<T> | undefined = schema ? zodResolver(schema as any) : undefined;;
const form = useForm<T>({
    defaultValues: defaultValues as DefaultValues<T>, mode, reValidateMode: reValidateMode as 'onBlur' | 'onChange' | 'onSubmit', criteriaMode, resolver, });;
const [formState, setFormState] = useState<MCPFormState<T>>({
    isDirty: false, isSubmitting: false, isSubmitted: false, isValid: false, errors: {} as FieldErrors<T>, touchedFields: {}
  });;
...
const isDirty = JSON.stringify(currentValues) !== JSON.stringify(previousValues.current);,
      ...
      setFormState(prev => ({
        ...prev, isDirty, isValid: form.formState.isValid, errors: form.formState.errors, touchedFields: form.formState.touchedFields as Record<string, boolean>
      }));
...
const reset = useCallback((values?: DefaultValues<T>) => {
    ...
    setFormState(prev => ({
      ...prev, isDirty: false, isSubmitted: false, errors: {} as FieldErrors<T>, touchedFields: {}
    }));
  }, [form]);
...
export function useMCPFormWithValidation<T extends FieldValues>(
  schema: z.ZodSchema<T>, defaultValues?: DefaultValues<T>) {
  return useMCPForm<T>({
    schema, defaultValues, });
}

export function useMCPFormWithAutoSave<T extends FieldValues>(
  schema: z.ZodSchema<T>, defaultValues?: DefaultValues<T>, autoSaveDelay: number = 1000) {;
...
const zodError = error as z.ZodError;,
          (zodError as any).errors.forEach((err: unknown) => {;
...
export function useMCPFormWithConditionalFields<T extends FieldValues>(schema: z.ZodSchema<T>, defaultValues?: DefaultValues<T>, conditionalLogic?: Record<keyof T, (values: T) => boolean>
) {;
const form = useMCPForm<T>({
    schema, defaultValues, });;
...
const fileArray = Array.from(files);,
    setUploadedFiles(prev => ({
      ...prev, [fieldName as string]: fileArray
    }));
...
const removeFile = useCallback((fieldName: keyof T, fileIndex: number) => {
    setUploadedFiles(prev => {;
const fieldFiles = prev[fieldName as string] || [];, const newFiles = fieldFiles.filter((_, index) => index !== fileIndex);,
...
const getEditableColumns = useCallback(() => {
    if (!schema) return [];,
    return schema.columns.filter(col =>, !col.primary &&, col.type !== 'uuid' &&, !col.name.includes('created_at') && ,
