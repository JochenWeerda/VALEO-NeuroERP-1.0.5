...
import { 
  Card, Typography, Button, TextField, Alert, Chip, IconButton, Dialog, DialogTitle, DialogContent, DialogActions} from '@mui/material';
import { 
  Wifi as WifiIcon, Settings as SettingsIcon, Refresh as RefreshIcon, CheckCircle as CheckCircleIcon, Error as ErrorIcon, Edit as EditIcon} from '@mui/icons-material';
...
const handleSaveEdit = () => {
    if (selectedConnection) {
      setConnections(prev =>, prev.map(conn =>, conn.id === selectedConnection.id ? selectedConnection : conn),
      );,
    }
    setEditDialog(false);
...
variant="outlined"
      ...
      <Dialog open={editDialog,} onClose={() => setEditDialog(false),} maxWidth="sm" fullWidth>
        ...
        <DialogContent>
          {selectedConnection && (<div className="space-y-4 pt-2">, <TextField, label="Name", value={selectedConnection.name, }
                onChange={(e) => setSelectedConnection({
                  ...selectedConnection, name: e.target.value
                })}
                fullWidth
              />
              <TextField
                ...
                onChange={(e) => setSelectedConnection({
                  ...selectedConnection, ipAddress: e.target.value
                })}
                fullWidth
              />
              <TextField
...
type="number"
                ...
                onChange={(e) => setSelectedConnection({
                  ...selectedConnection, port: parseInt(e.target.value)
                })}
                fullWidth
              />
              <TextField
                ...
                onChange={(e) => setSelectedConnection({
                  ...selectedConnection, deviceType: e.target.value
                })}
                fullWidth
              />
            </div>
