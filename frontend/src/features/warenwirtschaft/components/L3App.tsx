...
import { 
  PhoneAndroid as PhoneAndroidIcon, Add as AddIcon, Refresh as RefreshIcon, CheckCircle as CheckCircleIcon, PlayArrow as PlayArrowIcon, Stop as StopIcon} from '@mui/icons-material';
...
const [formData, setFormData] = useState({
    tournummer: '', kommissioniererId: '', kommissioniererName: '', geraetId: '', startdatum: new Date().toISOString().split('T')[0]
  });;
...
const handleStartTour = async (tourId: string) => {
    setTouren(touren.map(t =>, t.id === tourId, ? { ...t, status: 'aktiv', startdatum: new Date() }
        : t
    ));;
...
const handleCompleteTour = async (tourId: string) => {
    setTouren(touren.map(t =>, t.id === tourId, ? { ...t, status: 'abgeschlossen', enddatum: new Date() }
        : t
    ));;
const suggestions = await getAgentSuggestions(
...
className="text-blue-600"
                        ...
                        {tour.status === 'geplant' && (<IconButton, size="small", onClick={() => handleStartTour(tour.id),};
className="text-green-600"
                          >
                            <PlayArrowIcon />
                          </IconButton>
                        )}
                        {tour.status === 'aktiv' && (<IconButton, size="small", onClick={() => handleCompleteTour(tour.id),};
className="text-orange-600"
      ...
      <Dialog open={openDialog,} onClose={() => setOpenDialog(false),} maxWidth="md" fullWidth>
        ...
        <DialogContent>
          {dialogMode === 'create' ? (<div className="space-y-4">, <TextField, fullWidth, label="Tournummer", value={formData.tournummer, }
...
type="date"
            ...
            <div className="space-y-4">
              ...
              {selectedTour?.positionen.map((position, index) => (<div key={index, } className="border rounded p-3 mb-2 flex justify-between items-center">
                  <div className="flex items-center gap-2">
                    {position.gescannt ? (, <CheckCircleIcon className="text-green-600" />) : (
                  ...
                  {selectedTour.status === 'aktiv' && !position.gescannt && (<IconButton, size="small", className="text-blue-600", >, <PlayArrowIcon />, </IconButton>),}
                </div>
              ))}
            </div>
          )}
        ...
        <DialogActions>
          ...
          {dialogMode === 'create' && (<Button variant="contained" color="primary">, Tour erstellen, </Button>),}
        </DialogActions>
      </Dialog>
    </div>
  );
