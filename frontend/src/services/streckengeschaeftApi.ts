...
const formData = new FormData();,
    ...
    return this.request<T>(endpoint, {
      method: 'POST', headers: {}, // Let browser set Content-Type for FormData
      body: formData, });
  }
}
...
const response = await fetch(`${API_BASE_URL, }${ENDPOINTS.STECKENGESCHAEFT_EXPORT, }/libreoffice`, {
      method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ ...filter, format, }),
    });
  ...
  static async printStreckengeschaeft(
    id: string, printerId?: string, options?: {
      copies?: number;
    ...
    return apiClient.post<{ success: boolean; jobId?: string; error?: string }>(
      `${ENDPOINTS.STECKENGESCHAEFT_PRINT, }/${id, }`, { printerId, options, });
  }

  static async printMultiple(
    ids: string[], printerId?: string, options?: {
      copies?: number;
      orientation?: 'portrait' | 'landscape';
      paperSize?: 'A4' | 'A3' | 'letter';
      includeSummen?: boolean;
    ...
    return apiClient.post<{ success: boolean; jobIds?: string[]; errors?: string[] }>(
      `${ENDPOINTS.STECKENGESCHAEFT_PRINT, }/multiple`, { ids, printerId, options, });
  }
  ...
  static async sendEmail(
    ...
    return apiClient.post<{ success: boolean; messageId?: string; error?: string }>(
      `${ENDPOINTS.STECKENGESCHAEFT_EMAIL, }/${id, }`, emailData);
  }

  // WhatsApp Integration
  static async sendWhatsApp(
    id: string, phoneNumber: string, message?: string, includeAttachment?: boolean): Promise<{ success: boolean; messageId?: string; error?: string }> {
    return apiClient.post<{ success: boolean; messageId?: string; error?: string }>(
      `${ENDPOINTS.STECKENGESCHAEFT_WHATSAPP, }/${id, }`, { phoneNumber, message, includeAttachment, });
  }

  // Datei-Konvertierung
  static async convertFile(
    file: File, targetFormat: 'pdf' | 'excel' | 'word' | 'libreoffice' | 'html' | 'txt', options?: {
...
const formData = new FormData();,
  ...
  static async batchUpdate(
    ids: string[], updates: Partial<Streckengeschaeft>): Promise<{ success: boolean; updated: number; errors: string[] }> {
    return apiClient.post<{ success: boolean; updated: number; errors: string[] }>(
      `${ENDPOINTS.STECKENGESCHAEFT, }/batch-update`, { ids, updates, });
  }
  ...
  static async batchDelete(ids: string[]): Promise<{ success: boolean; deleted: number; errors: string[] }> {
    return apiClient.post<{ success: boolean; deleted: number; errors: string[] }>(
      `${ENDPOINTS.STECKENGESCHAEFT, }/batch-delete`, { ids, });
  }
  ...
  static async restoreBackup(backupId: string): Promise<{ success: boolean; restored: number }> {
    return apiClient.post<{ success: boolean; restored: number }>(
      `${ENDPOINTS.STECKENGESCHAEFT, }/restore`, { backupId, });
  }
}
