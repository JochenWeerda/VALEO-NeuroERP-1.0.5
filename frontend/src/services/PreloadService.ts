...
const loadComponent = async (): Promise<T> => {
    ...
    loadingPromise = importFunc().then(module => {
      Component = module.default;, const loadTime = performance.now() - (loadStartTime || 0);,
      
      // Performance-Metrik speichern,
      preloadService.recordPerformanceMetric({
        route: routeName, loadTime, bundleSize: config?.estimatedSize || 0, cacheHit: false, timestamp: Date.now()
      });
...
const totalSize = Object.values(CRITICAL_ROUTES).reduce((sum, config) => 
    ...
    this.bundleAnalysis = {
      ...
      largestChunks: Object.entries(CRITICAL_ROUTES)
        .map(([route, config]) => ({
          name: route, size: config.estimatedSize || 0, percentage: ((config.estimatedSize || 0) / totalSize) * 100
        }))
...
const { _totalSize, _largestChunks,} = this.bundleAnalysis;
...
const startTime = performance.now();,
    ...
    this.recordPerformanceMetric({
      route, loadTime: performance.now() - startTime,
...
export const usePreload = () => {
  return {
    preloadRoute: (route: _string) => preloadService.preloadRoute(route),
