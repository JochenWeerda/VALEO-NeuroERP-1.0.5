...
const { _error,} = await supabase
        .from('nachrichten')
        .update({ 
          gelesenVon: updatedGelesenVon, status: 'gelesen'
        })
        .eq('id', nachrichtenId);
...
const { _error,} = await supabase
        .from('nachrichten')
        .update({ 
          bestaetigtVon: updatedBestaetigtVon, status: 'bestaetigt'
        })
        .eq('id', nachrichtenId);
...
const protokoll: Tagesprotokoll = {
      ...
      if (bestellungen) {
        bestellungen.forEach(bestellung => {
          protokoll.details.push({
            typ: 'bestellung', beschreibung: `Bestellung ${bestellung.nummer} - ${bestellung.lieferant, }`, zeitpunkt: new Date(bestellung.erstellt_am),
      ...
      if (lieferungen) {
        lieferungen.forEach(lieferung => {
          protokoll.details.push({
            typ: 'lieferung', beschreibung: `Lieferung ${lieferung.nummer} - ${lieferung.lieferant, }`, zeitpunkt: new Date(lieferung.erstellt_am),
      ...
      if (rechnungen) {
        rechnungen.forEach(rechnung => {
          protokoll.details.push({
            typ: 'rechnung', beschreibung: `Rechnung ${rechnung.nummer} - ${rechnung.kunde, }`, zeitpunkt: new Date(rechnung.erstellt_am),
...
const vorschlaege: KINachrichtenVorschlag[] = [];
      ...
      if (kontext?.modul === 'neuroflow') {
        vorschlaege.push({
          betreff: 'NeuroFlow System-Update', inhalt: 'Das NeuroFlow-System wurde erfolgreich aktualisiert. Alle Funktionen sind verfügbar.', empfaengerGruppe: 'neuroflow', prioritaet: 'normal', konfidenz: 0.85, begruendung: 'Automatische Benachrichtigung nach System-Update'
        });
      }

      if (kontext?.prozess === 'bestellung') {
        vorschlaege.push({
          betreff: 'Bestellbestätigung', inhalt: 'Ihre Bestellung wurde erfolgreich verarbeitet und wird vorbereitet.', empfaengerGruppe: 'allgemein', prioritaet: 'normal', konfidenz: 0.90, begruendung: 'Standard-Bestellbestätigung'
        });
      }
...
const { _data, _error,} = await supabase
      ...
      return data.map(user => ({
        id: user.id, name: user.name, email: user.email, gruppe: user.gruppe as EmpfaengerGruppe, rolle: user.rolle as any
      }));
...
const slowestOperations = Array.from(operationStats.entries())
      .map(([operation, times]) => ({
        operation, averageTime: times.reduce((sum, time) => sum + time, 0) / times.length
      }))
