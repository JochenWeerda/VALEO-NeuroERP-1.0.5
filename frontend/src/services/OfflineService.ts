...
class OfflineService {
  ...
  private listeners: ((status: _SyncStatus) => void)[] = [];;
constructor() {
    this.initDatabase();,
...
const store = db.createObjectStore('syncStatus', { keyPath: 'key' });
  ...
  async saveOfflineData(type: OfflineData['type'], key: string, data: unknown): Promise<void> {
...
const response = await fetch(request.url, {
            method: request.method, headers: request.headers, body: request.body ? JSON.stringify(request.body) : undefined
          });
  ...
  async offlineFirstRequest<T>(
    url: string, options: RequestInit = {}, offlineDataKey?: string): Promise<T> {
    try {
...
const offlineData = await this.getOfflineData('product', offlineDataKey);,
      ...
      await this.addPendingRequest({
        url, method: options.method || 'GET', headers: options.headers as Record<string, string> || {}, body: options.body
      });
...
const status: SyncStatus = {
  ...
  addStatusListener(listener: (status: _SyncStatus) => void): void {
    this.listeners.push(listener);,
  }

  removeStatusListener(listener: (status: _SyncStatus) => void): void {;
