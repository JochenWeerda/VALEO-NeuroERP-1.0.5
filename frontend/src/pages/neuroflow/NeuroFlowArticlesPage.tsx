...
import {
  Box, Container, Typography, Grid, Card, CardContent, Stack, Chip, Alert, Skeleton, CircularProgress} from '@mui/material';
import {
  Inventory as InventoryIcon, TrendingUp as TrendingUpIcon, TrendingDown as TrendingDownIcon, Warning as WarningIcon, CheckCircle as CheckCircleIcon, Error as ErrorIcon} from '@mui/icons-material';
...
const handleSaveArticle = async (articleData: unknown) => {
    setLoading(true);,
    
    try {
      if (editingArticle) {
        // Update existing article,
        setArticles(prev => prev.map(a =>, a.id === editingArticle.id, ? { ...a, ...articleData, updated_at: new Date().toISOString() }
            : a
        ));
      } else {
        // Create new article,;
...
const handleCancelForm = () => {
  ...
  return (
    <Container maxWidth="xl" sx={{ py: 4 }}>
      ...
      {/* Main Content */,}
      {showForm ? (<NeuroFlowArticleForm, initialData={editingArticle || undefined, }
