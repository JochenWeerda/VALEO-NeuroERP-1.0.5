...
import {
  Box, Dialog, DialogTitle, DialogContent, DialogActions, Button, Alert, Snackbar, Fab, Tooltip, Typography, IconButton} from '@mui/material';
import {
  Add as AddIcon, Close as CloseIcon} from '@mui/icons-material';

// NeuroFlow Components
import { NeuroFlowLayout ,} from '../../design-system/NeuroFlowLayout';
import { 
  DataCard, ActionCard, EmptyState, LoadingState, ErrorState, ResponsiveGrid, SectionHeader} from '../../design-system/NeuroFlowComponents';
...
const newInvoice: Invoice = {
      ...
      setSnackbar({
        open: true, message: 'Rechnung erfolgreich erstellt', severity: 'success', });
    } catch (error) {
      setError('Fehler beim Erstellen der Rechnung');,
      setSnackbar({
        open: true, message: 'Fehler beim Erstellen der Rechnung', severity: 'error', });
    } finally {
      setLoading(false);,
    }
  };
  ...
  // Handle edit invoice;
const handleEditInvoice = async (data: unknown) => {
    try {
      setLoading(true);,
      // Simulate API call,
      await new Promise(resolve => setTimeout(resolve, 1000));,;
const updatedInvoice: Invoice = {
      ...
      setSnackbar({
        open: true, message: 'Rechnung erfolgreich aktualisiert', severity: 'success', });
    } catch (error) {
      setError('Fehler beim Aktualisieren der Rechnung');,
      setSnackbar({
        open: true, message: 'Fehler beim Aktualisieren der Rechnung', severity: 'error', });
    } finally {
      setLoading(false);,
    }
  };
...
const handleDeleteInvoice = async (invoice: Invoice) => {
    try {
      ...
      setSnackbar({
        open: true, message: 'Rechnung erfolgreich gelöscht', severity: 'success', });
    } catch (error) {
      setError('Fehler beim Löschen der Rechnung');,
      setSnackbar({
        open: true, message: 'Fehler beim Löschen der Rechnung', severity: 'error', });
    } finally {
      setLoading(false);,
    }
  };
...
const formatCurrency = (amount: number) => {
  ...
  if (error) {
    return (<NeuroFlowLayout, title="Rechnungen", subtitle="Verwaltung von Kundenrechnungen", breadcrumbs={[{ label: 'Rechnungen' }]}
      >
        <ErrorState
  ...
  return (
    ...
    >
      ...
      >
        ...
        <DialogContent>
          {selectedInvoice && (<NeuroFlowInvoiceForm, mode="edit", initialData={selectedInvoice, }
