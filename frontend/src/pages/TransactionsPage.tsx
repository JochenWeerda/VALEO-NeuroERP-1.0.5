...
import { 
  Box, Card, Typography, Button, Chip, Table, TableBody, TableCell, TableContainer, TableHead, TableRow, Paper, Dialog, DialogTitle, DialogContent, DialogActions, TextField, FormControl, InputLabel, Select, MenuItem, IconButton, Tooltip, Alert, CircularProgress, TablePagination, InputAdornment} from '@mui/material';
import {
  Add as AddIcon, Edit as EditIcon, Delete as DeleteIcon, Search as SearchIcon, Refresh as RefreshIcon, TrendingUp as TrendingUpIcon, TrendingDown as TrendingDownIcon, AccountBalance as AccountBalanceIcon} from '@mui/icons-material';
...
const {
    _transactions, _getTransactions, _createTransaction, _isLoading, _error,} = useApi();;
const [openDialog, setOpenDialog] = useState(false);;
const [editingTransaction, setEditingTransaction] = useState<any>(null);;
const [formData, setFormData] = useState<TransactionFormData>({;
type: 'income', amount: 0, description: '', date: new Date().toISOString().split('T')[0]
  });;
...
type: formData.type as 'income' | 'expense', amount: formData.amount, description: formData.description, date: formData.date, user_id: localStorage.getItem('userId') || 'current-user-id',
          status: 'pending'
...
const handleEdit = (transaction: unknown) => {
    ...
    setFormData({;
type: transaction.type, amount: transaction.amount, description: transaction.description || '', date: new Date(transaction.date).toISOString().split('T')[0]
    });
    setOpenDialog(true);
  };;
const resetForm = () => {
