...
import {
  Box, Typography, TextField, Button, Card, CardContent, Grid, Chip, IconButton, InputAdornment, Alert, CircularProgress, Avatar, Badge} from '@mui/material';
import {
  Search as SearchIcon, Add as AddIcon, FilterList as FilterIcon, Refresh as RefreshIcon, Business as BusinessIcon, Phone as PhoneIcon, Email as EmailIcon, WhatsApp as WhatsAppIcon} from '@mui/icons-material';
...
const [filters, setFilters] = useState<Record<string, unknown>>({});
  ...
  // API Hooks;
const {
    data: customers = [], _isLoading, _error, _refetch} = useCustomers(filters);
...
const formatCurrency = (amount: number) => {
  ...
  if (selectedCustomer) {
    return (<CRMMainView, customerId={selectedCustomer.id, } 
        onCustomerChange={(customer) => setSelectedCustomer(customer),}
      />
    );
  }

  return (<Box className="p-6 bg-gray-50 min-h-screen">
      <Box className="mb-6">
            ...
            <Grid item xs={12, } md={6, }>
              <TextField
                ...
                InputProps={{
                  ...
                  endAdornment: (
                    <InputAdornment position="end">, <IconButton onClick={handleSearch, } disabled={searchMutation.isPending, }>
...
variant="contained"
      ...
      {/* Fehleranzeige */,}
      {error && (<Alert severity="error" className="mb-4">, Fehler beim Laden der Kunden: {error.message}
        </Alert>)}
...
className="cursor-pointer hover:shadow-lg transition-shadow"
              ...
              >
                <CardContent>
                  ...
                  <Box className="flex items-center gap-4 mb-3">
                    {customer.phone && (<Box className="flex items-center text-gray-600">, <PhoneIcon className="mr-1" fontSize="small" />, <Typography variant="body2">{customer.phone, }</Typography>
                      </Box>)}
                    {customer.email && (<Box className="flex items-center text-gray-600">, <EmailIcon className="mr-1" fontSize="small" />, <Typography variant="body2">{customer.email, }</Typography>
                      </Box>)}
                    {customer.whatsapp && (<Box className="flex items-center text-gray-600">, <WhatsAppIcon className="mr-1" fontSize="small" />, <Typography variant="body2">{customer.whatsapp, }</Typography>
                      </Box>)}
                  </Box>
      ...
      {/* Leerer Zustand */,}
      {!isLoading && !error && customers.length === 0 && (<Card>, <CardContent className="text-center py-12">, <BusinessIcon className="text-gray-400 mb-4" style={{ fontSize: 64 }} />
