...
const [assetForm, setAssetForm] = useState({
    bezeichnung: '', kategoriename: '', anschaffungsdatum: '', anschaffungswert: '', standort: '', status: 'Aktiv'
  });;
const [vehicleForm, setVehicleForm] = useState({
    kennzeichen: '', marke: '', modell: '', baujahr: '', kilometerstand: '', tuev_bis: '', versicherung_bis: '', steuer_bis: ''
  });;
const [maintenanceForm, setMaintenanceForm] = useState({
    anlage_id: '', wartungstyp: '', geplantes_datum: '', prioritaet: 'Normal', beschreibung: ''
  });
                ...
                {assets.map((asset) => (<TableRow key={asset.id, }>
                    ...
                    <TableCell>
                      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                        ...
                        {asset.naechste_wartung !== 'Keine geplant' && (<Chip, label="Wartung geplant", color="warning", size="small", icon={<ScheduleIcon />, }
                          />)}
                      </Box>
                    </TableCell>
                    <TableCell>
            ...
            <Grid item xs={12,}>
              <Card>
                <CardContent>
                  ...
                  <Grid container spacing={2,}>
                    {assets.filter(asset =>, asset.naechste_wartung !== 'Keine geplant' &&, new Date(asset.naechste_wartung) <= new Date(Date.now() + 30 * 24 * 60 * 60 * 1000),
