...
import {
  Box, Card, Typography, Button, TextField, FormControl, InputLabel, Select, MenuItem, Grid, Dialog, DialogTitle, DialogContent, DialogActions, IconButton, Chip, Divider, List, ListItem, ListItemIcon, ListItemText, Stepper, Step, StepLabel, StepContent, Alert, Snackbar, Tabs, Tab, Table, TableBody, TableCell, TableContainer, TableHead, TableRow, Paper} from '@mui/material';
import {
  Add as AddIcon, Edit as EditIcon, Delete as DeleteIcon, Save as SaveIcon, Person as PersonIcon, Business as BusinessIcon, Email as EmailIcon, Phone as PhoneIcon, LocationOn as LocationIcon, AccountBalance as AccountBalanceIcon, Assignment as AssignmentIcon, Receipt as ReceiptIcon, History as HistoryIcon, Notes as NotesIcon, CheckCircle as CheckCircleIcon, Warning as WarningIcon, Error as ErrorIcon} from '@mui/icons-material';;
...
const [kundeForm, setKundeForm] = useState({
    kundennummer: '', typ: 'geschaeft' as Kunde['typ'], name: '', ansprechpartner: '', email: '', telefon: '', strasse: '', plz: '', ort: '', land: 'Deutschland', steuernummer: '', ust_id: '', zahlungsbedingungen: '30 Tage netto', kreditlimit: 0, status: 'aktiv' as Kunde['status'], kategorie: 'Standard', notizen: ''
  });

  // Mock-Daten für Kunden;
const [kunden, setKunden] = useState<Kunde[]>([
    {
      id: '1', kundennummer: 'K-2024-001', typ: 'geschaeft', name: 'Müller GmbH', ansprechpartner: 'Max Müller', email: 'max.mueller@mueller-gmbh.de', telefon: '+49 89 1234 5678', adresse: {
        strasse: 'Musterstraße 123', plz: '80331', ort: 'München', land: 'Deutschland'
      }, steuernummer: '143/123/12345', ust_id: 'DE123456789', zahlungsbedingungen: '30 Tage netto', kreditlimit: 50000, status: 'aktiv', kategorie: 'Premium', notizen: 'Wichtiger Kunde, bevorzugte Behandlung', erstellt_am: '2024-01-15', letzte_aktivitaet: '2024-01-20'
    }, {
      id: '2', kundennummer: 'K-2024-002', typ: 'privat', name: 'Schmidt, Anna', email: 'anna.schmidt@email.de', telefon: '+49 40 9876 5432', adresse: {
        strasse: 'Beispielweg 456', plz: '20095', ort: 'Hamburg', land: 'Deutschland'
      }, zahlungsbedingungen: 'Sofort', kreditlimit: 1000, status: 'aktiv', kategorie: 'Standard', notizen: 'Privatkunde', erstellt_am: '2024-01-16', letzte_aktivitaet: '2024-01-18'
    }
  ]);;
...
const handleOpenDialog = (kunde?: Kunde) => {
    if (kunde) {
      ...
      setKundeForm({
        kundennummer: kunde.kundennummer, typ: kunde.typ, name: kunde.name, ansprechpartner: kunde.ansprechpartner || '', email: kunde.email, telefon: kunde.telefon, strasse: kunde.adresse.strasse, plz: kunde.adresse.plz, ort: kunde.adresse.ort, land: kunde.adresse.land, steuernummer: kunde.steuernummer || '', ust_id: kunde.ust_id || '', zahlungsbedingungen: kunde.zahlungsbedingungen, kreditlimit: kunde.kreditlimit, status: kunde.status, kategorie: kunde.kategorie, notizen: kunde.notizen
      });
    } else {
      setSelectedKunde(null);,
      setKundeForm({
        kundennummer: '', typ: 'geschaeft', name: '', ansprechpartner: '', email: '', telefon: '', strasse: '', plz: '', ort: '', land: 'Deutschland', steuernummer: '', ust_id: '', zahlungsbedingungen: '30 Tage netto', kreditlimit: 0, status: 'aktiv', kategorie: 'Standard', notizen: ''
      });
    }
    setOpenDialog(true);
  };;
...
const updateKontakt = (id: string, field: keyof KundenKontakt, value: unknown) => {
                  ...
                  {kunden.map((kunde) => (<TableRow key={kunde.id, } hover>
                      ...
                      <TableCell>
                        <Box>
                          ...
                          {kunde.ansprechpartner && (, <Typography variant="caption" color="text.secondary">, {kunde.ansprechpartner, }
                            </Typography>)}
                        </Box>
                      </TableCell>
                      <TableCell>
