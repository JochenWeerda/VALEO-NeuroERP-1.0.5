...
import {
  Container, Typography, Box, Card, CardContent, Grid, Button, Tabs, Tab, Dialog, DialogTitle, DialogContent, DialogActions, List, ListItem, ListItemText, ListItemSecondaryAction, IconButton, Chip, Alert, CircularProgress, Paper, Divider} from '@mui/material';
import {
  Add as AddIcon, Edit as EditIcon, Delete as DeleteIcon, Visibility as ViewIcon, Refresh as RefreshIcon, Dataset as DatabaseIcon, Api as ApiIcon, CheckCircle as CheckCircleIcon, Error as ErrorIcon, Warning as WarningIcon} from '@mui/icons-material';
...
const [state, setState] = useState<DemoState>({
    selectedModule: 'warenwirtschaft', selectedForm: null, formData: [], loading: false, error: null, dialogOpen: false, dialogMode: 'create', selectedRecord: null
  });;
...
const response = await formDataService.getFormDataList(formConfig);,
      if (response.success && response.data) {
        setState(prev => ({ 
          ...prev, formData: response.data, loading: false 
        }));
        message.success(`${response.data.length, } Datensätze geladen`);
      } else {
        setState(prev => ({ 
          ...prev, error: 'Fehler beim Laden der Daten', loading: false 
        }));
...
const handleOpenDialog = (mode: 'create' | 'edit' | 'view', record?: unknown) => {
    setState(prev => ({
      ...prev, dialogOpen: true, dialogMode: mode, selectedRecord: record || null
    }));
  };;
const handleCloseDialog = () => {
    setState(prev => ({
      ...prev, dialogOpen: false, selectedRecord: null
    }));
  };
...
const handleRecordDelete = async (record: unknown) => {
    if (!state.selectedForm) return;,
          ...
          <TabPanel value={tabValue,} index={0,}>
            <Grid container spacing={3,}>
              ...
              <Grid item xs={12,} md={8,}>
                {state.selectedForm ? (<Card>, <CardContent>, <Box display="flex" justifyContent="space-between" alignItems="center" mb={2, }>
...
variant="outlined"
        ...
        >
          ...
          <DialogContent>
            {state.selectedForm && (<ModernERPFormWithDB, config={state.selectedForm, }
