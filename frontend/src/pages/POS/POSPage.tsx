...
import {
  Box, Grid, Card, CardContent, Typography, Button, TextField, IconButton, Chip, Divider, Alert, CircularProgress, Dialog, DialogTitle, DialogContent, DialogActions, Select, MenuItem, FormControl, InputLabel} from '@mui/material';
import {
  ShoppingCart as CartIcon, Add as AddIcon, Remove as RemoveIcon, Clear as ClearIcon, Receipt as ReceiptIcon, Payment as PaymentIcon, Search as SearchIcon, Category as CategoryIcon, Euro as EuroIcon, LocalOffer as DiscountIcon} from '@mui/icons-material';;
interface Product {
  artikel_nr: string;
  bezeichnung: string;
  kurztext: string;
...
const response = await fetch('/api/pos/cart/add', {
        method: 'POST', headers: {
          ...
          artikel_nr: product.artikel_nr, menge: quantity
        })
      });
...
const response = await fetch('/api/pos/cart/remove', {
        method: 'DELETE', headers: {
          'Content-Type': 'application/json', }, body: JSON.stringify({
          artikel_nr: artikelNr, menge: quantity
        })
      });
...
const categories = [...new Set(products.map(p => p.kategorie).filter(Boolean))];
  ...
  return (<Box sx={{ p: 3, height: '100vh', display: 'flex', flexDirection: 'column' }}>
            ...
            <CardContent sx={{ flex: 1, overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
              ...
              <Box sx={{ flex: 1, overflow: 'auto' }}>
                {loading ? (<Box sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: '100%' }}>
                  ...
                  <Grid container spacing={2, }>
                    {filteredProducts.map((product) => (<Grid item xs={6, } sm={4, } md={3, } key={product.artikel_nr, }>
                        <Card 
                          sx={{ 
                            cursor: 'pointer', '&:hover': { boxShadow: 3 }, height: '100%', display: 'flex', flexDirection: 'column'
                          }}
                        ...
                        >
                          <CardContent sx={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
                            ...
                            {product.lagerbestand <= 0 && (<Chip, label="Nicht verfügbar", color="error", size="small", sx={{ mt: 1 }}
                              />)}
                          </CardContent>
                        </Card>
                      </Grid>
          ...
          <Card sx={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
            <CardContent sx={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
              ...
              <Box sx={{ flex: 1, overflow: 'auto', mb: 2 }}>
                {cart.length === 0 ? (<Typography color="text.secondary" align="center">, Warenkorb ist leer, </Typography>) : (
        ...
        <DialogContent>
          ...
          <FormControl fullWidth sx={{ mb: 2 }}>
            ...
            <Select
              ...
              label="Zahlungsart"
            >
              {paymentMethods.map(method => (, <MenuItem key={method.value, } value={method.value, }>
                  {method.label, }
                </MenuItem>))}
            </Select>
          </FormControl>
