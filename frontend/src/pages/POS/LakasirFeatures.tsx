...
import {
  Box, Tabs, Tab, Typography, Card, Container, Paper} from '@mui/material';
import {
  QrCode as BarcodeIcon, Inventory as InventoryIcon, LocalOffer as VoucherIcon} from '@mui/icons-material';
...
const { _children, _value, _index, _...other,} = props;
  ...
  return (
    <div
      ...
      {...other, }
    >
      {value === index && (, <Box className="py-4">, {children, }
        </Box>)}
    </div>
  );
};
...
variant="scrollable"
          ...
          >
            <Tab
              label={
                <Box className="flex items-center gap-2">, <BarcodeIcon />, <span>Barcode-Scanner</span>, </Box>, }
              {...a11yProps(0),}
            />
            <Tab
              label={
