...
import {
  Card, CardContent, Typography, Box, Button, IconButton, Dialog, DialogTitle, DialogContent, DialogActions, Chip, Avatar, LinearProgress, Tooltip, Switch, FormControlLabel, Divider, Alert} from '@mui/material';
import {
  Dashboard as DashboardIcon, Settings as SettingsIcon, Add as AddIcon, Remove as RemoveIcon, DragIndicator as DragIcon, Business as BusinessIcon, Inventory as InventoryIcon, Receipt as ReceiptIcon, Description as DocumentIcon, Analytics as AnalyticsIcon, People as PeopleIcon, ShoppingCart as SalesIcon, LocalShipping as WarehouseIcon, Build as ProductionIcon, AccountBalance as FinanceIcon, Assessment as QualityIcon, Assignment as ProjectIcon, Notifications as NotificationsIcon, TrendingUp as TrendingUpIcon, TrendingDown as TrendingDownIcon, Warning as WarningIcon, CheckCircle as SuccessIcon, Error as ErrorIcon, Schedule as ScheduleIcon, Star as StarIcon, Favorite as FavoriteIcon, Visibility as VisibilityIcon, VisibilityOff as VisibilityOffIcon, LocalShipping as LocalShippingIcon, Build as BuildIcon, Assessment as AssessmentIcon} from '@mui/icons-material';
...
const { _user, _isAuthenticated,} = useApi();;
...
const userRole = getUserRole();,
        userComponents = availableComponents.filter(comp =>, !comp.roleBased || comp.allowedRoles.includes(userRole),
        ).map((comp, index) => ({
          ...comp, position: index + 1, visible: true
        }));
      }
...
const updatedComponents = components.map(comp =>, comp.id === componentId ? { ...comp, visible: !comp.visible } : comp);
    saveDashboardConfig(updatedComponents);
  };;
const toggleFavorite = (componentId: string) => {;
const updatedComponents = components.map(comp =>, comp.id === componentId ? { ...comp, favorite: !comp.favorite } : comp);
    saveDashboardConfig(updatedComponents);
  };;
...
const renderComponent = (component: DashboardComponent) => {
    ...
    switch (component.type) {
      case 'metric':
        return (<DataCard, key={component.id, }
            ...
            trend={component.data.trend ? {
              value: component.data.change, direction: component.data.trend
            } : undefined}
      ...
      case 'quick-action':
        return (<Card key={component.id, } className="h-full">
            <CardContent>
              ...
              <Box className="grid grid-cols-2 gap-2">
                {component.data.actions.map((action: unknown, index: number) => (
                  <Button, key={index, }
                    fullWidth;
variant="outlined"
        ...
        return (<Card key={component.id, } className="h-full">
            <CardContent>
              ...
              <Box className="space-y-2">
                {component.data.notifications.map((notification: unknown) => (
                  <Alert, key={notification.id, }
                    severity={notification.type, };
className="text-sm"
                    action={
                      <Typography variant="caption" className="text-gray-500">, {notification.time, }
                      </Typography>
                    }
                  >
      ...
      case 'favorite':
        return (<Card key={component.id, } className="h-full">
            <CardContent>
              ...
              <Box className="grid grid-cols-2 gap-2">
                {component.data.favorites.map((favorite: unknown, index: number) => (
                  <Button, key={index, }
                    fullWidth;
variant="outlined"
...
const getGridSize = (size: string) => {
  ...
  if (loading) {
    return (<Box className="flex items-center justify-center h-64">, <LinearProgress className="w-32" />, </Box>);,
  }
  ...
  return (
    <Box className="p-6 bg-gray-50 min-h-screen">
      ...
      <SectionHeader
        ...
        actions={
          <Box className="flex items-center space-x-2">, <Chip, icon={<BusinessIcon />, }
...
variant="outlined"
      ...
      <Box className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {components,
          ...
          .map(component => (, <Box key={component.id, } className={`w-full ${getGridSize(component.size),}`}>
                ...
                return (<Card key={component.id, } variant="outlined">
                    <CardContent>
                      <Box className="flex items-center justify-between">
                        ...
                        <Box className="flex items-center space-x-1">
                          {component.roleBased && (, <Chip, label="Rollenbasiert", size="small", variant="outlined", color="primary", />),}
                          {isAdded ? (<Button, size="small", variant="outlined", color="error", startIcon={<RemoveIcon />, }
...
variant="contained"
            ...
            <Box className="space-y-2">
              {components.map(component => (, <Box key={component.id, } className="flex items-center justify-between p-3 border rounded">
                  <Box className="flex items-center">
                  ...
                  <Box className="flex items-center space-x-2">
                    <FormControlLabel
                      control={
                        <Switch, checked={component.visible, }
...
const defaultComponents = availableComponents,
                ...
                .map((comp, index) => ({
                  ...comp, position: index + 1, visible: true, favorite: false
                }));
