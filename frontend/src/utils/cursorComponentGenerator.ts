...
const handleCancel = () => {
  ...
  return (
    <Card className="max-w-2xl mx-auto shadow-lg">
      <CardHeader
        title={
          <Box className="flex items-center space-x-2">, <AssignmentIcon className="text-blue-600" />, <Typography variant="h5" component="h2">, {initialData?.id ? '${this.getLabel(tableName, 'edit', language),}' : '${this.getLabel(tableName, 'create', language),}'}
            </Typography>
          </Box>
        }
...
export const ${className,}Table: React.FC<${className,}TableProps> = ({
  ${tableName, }s, isLoading = false, onView, onEdit, onDelete
}) => {;
...
let filtered = ${tableName,}s.filter(item => {
      return searchTerm === '' ||, Object.values(item).some(value =>, value?.toString().toLowerCase().includes(searchTerm.toLowerCase()),
        );,
    });
...
${tableColumns,}
          ...
          <TableBody>
            {paginatedData.length === 0 ? (<TableRow>, <TableCell colSpan={${schema.columns.length + 1, }} align="center" className="py-8">
                  <Alert severity="info">
...
`;
  ...
  private generateFormField(col: unknown, schema: MCPSchema, language: string): string {
...
type="${this.getFieldType(col.type),}"
  ...
  private generateTableColumn(col: unknown, schema: MCPSchema, language: string): string {
    return `              <TableCell>,
                <TableSortLabel,
...
const labels: Record<string, Record<string, string>> = {
  ...
  private getColumnDescription(col: unknown, language: string): string {
