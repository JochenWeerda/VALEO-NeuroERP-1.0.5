...
import('quagga').then(module => {
  Quagga = module.default || module;, }).catch(() => {
  console.warn('Quagga nicht verfügbar - Mock verwendet');,
});;
interface BarcodeScannerProps {
  onBarcodeDetected: (barcode: _string) => void;
...
export const BarcodeScanner: React.FC<BarcodeScannerProps> = ({
  onBarcodeDetected, onError, autoStart = false, className = '', }) => {;
...
const initializeScanner = useCallback(async () => {
    ...
    try {
      ...
      await Quagga.init({
        inputStream: {
          name: "Live", type: "LiveStream", target: scannerRef.current, constraints: {
            width: { min: 640, ideal: 1280, max: 1920 }, height: { min: 480, ideal: 720, max: 1080 }, facingMode: "environment" // Rückkamera bevorzugen
          }, }, locator: {
          patchSize: "medium", halfSample: true
        }, numOfWorkers: navigator.hardwareConcurrency || 4, frequency: 10, decoder: {
          readers: [
            "ean_reader", "ean_8_reader", "code_128_reader", "code_39_reader", "upc_reader", "upc_e_reader", ], }, locate: true
      });
    ...
    <Card className={`p-4 ${className, }`}>
      <Box className="flex items-center justify-between mb-4">
        ...
        <Box className="flex gap-2">
          {!isScanning ? (, <Button, variant="contained", startIcon={<CameraIcon />, }
...
variant="outlined"
      ...
      {lastScannedCode && (<Alert severity="success" className="mb-4">, Barcode erkannt: <strong>{lastScannedCode}</strong>
        </Alert>)}
...
className="w-full h-64 bg-gray-100 rounded-lg overflow-hidden border-2 border-gray-300"
        >
          {!isScanning && (<Box className="flex items-center justify-center h-full">, <Box className="text-center">, <CameraIcon className="text-4xl text-gray-400 mb-2" />, <Typography variant="body2" color="textSecondary">, Scanner bereit - Klicken Sie auf "Scanner starten", </Typography>, </Box>, </Box>),}
        </div>

        {isScanning && (<Box className="absolute inset-0 flex items-center justify-center">, <CircularProgress size={40, } />
          </Box>)}
      </Box>
