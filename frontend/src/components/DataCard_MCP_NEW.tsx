...
import { 
  Card, CardContent, Typography, Box, Chip, CircularProgress, Tooltip} from '@mui/material';
import {
  TrendingUp as TrendingUpIcon, TrendingDown as TrendingDownIcon, Remove as RemoveIcon, Info as InfoIcon, Warning as WarningIcon, CheckCircle as CheckCircleIcon} from '@mui/icons-material';
...
export const DataCard_MCP_NEW: React.FC<DataCardProps> = ({
  title, dataSource, valueField, trendField, icon, trustLevel, confidence, refreshInterval = 300 // 5 Minuten Standard, }) => {;
...
const newDataPoint: DataPoint = {
        ...
        setPreviousData(previous ? {
          value: previousValue, timestamp: (previous as any).created_at || new Date().toISOString()
        } : null);
      } else {
    ...
    <Card sx={{ height: '100%', '&:hover': { boxShadow: 3 }, transition: 'box-shadow 0.2s' }}>
      <CardContent>
        ...
        <Box display="flex" flexDirection="column" gap={0.5,}>
          ...
          {previousData && (<Typography variant="caption" color="text.secondary">, {UI_LABELS.MESSAGES.PREVIOUS_VALUE, }: {previousData.value.toLocaleString('de-DE'),} 
