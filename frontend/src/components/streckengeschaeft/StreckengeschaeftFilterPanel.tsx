...
import { 
  Form, Input, Select, DatePicker, InputNumber, Button, Card, Collapse, Checkbox, Row, Col, Divider, Tabs, Typography, Space} from 'antd';
import { 
  FilterOutlined, SearchOutlined, ClearOutlined, ReloadOutlined} from '@ant-design/icons';
import { 
  StreckengeschaeftFilter, VorgangsTyp, StreckenStatus, BiomasseOption, getVorgangsTypLabel, getStatusLabel, getBiomasseOptionLabel} from '../../types/streckengeschaeft';;
const { _TabPane,} = Tabs;;
const { _RangePicker,} = DatePicker;;
const { _Option,} = Select;;
interface StreckengeschaeftFilterPanelProps {
  onFilterChange: (filter: _StreckengeschaeftFilter) => void;
  onReset: () => void;
  loading?: boolean;
}
...
const handleFilterChange = (allValues: unknown) => {;
const { _datumRange, _...restValues,} = allValues;;
...
className="mb-4 shadow-sm"
      title={
        <Space>, <FilterOutlined />, <span>Filter für Streckengeschäfte</span>, </Space>, }
      extra={
        <Space>, <Button, type="text", icon={expanded ? <FilterOutlined /> : <FilterOutlined />}
                  ...
                  <Col span={12,}>
                    <Form.Item label="EK/VK nach Biomasse" name="biomasseOption">
                      <Select placeholder="Biomasse-Option wählen">
                        {biomasseOptions.map(option => (, <Option key={option.value, } value={option.value, }>
                            {option.label, }
                          </Option>))}
                      </Select>
                    </Form.Item>
                  ...
                  <Col span={12,}>
                    <Form.Item label="Vorgangstyp" name="vorgangsTyp">
                      <Select placeholder="Vorgangstyp wählen" allowClear>
                        {vorgangsTypOptions.map(option => (, <Option key={option.value, } value={option.value, }>
                            {option.label, }
                          </Option>))}
                      </Select>
                    </Form.Item>
