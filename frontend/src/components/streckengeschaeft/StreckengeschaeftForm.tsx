...
import {
  Form, Input, Select, DatePicker, InputNumber, Button, Row, Col, Card, Typography, Divider, Tabs, Checkbox, Space, message} from 'antd';
import {
  SaveOutlined, CloseOutlined, CalculatorOutlined} from '@ant-design/icons';
import { useForm, Controller} from 'react-hook-form';
import { zodResolver ,} from '@hookform/resolvers/zod';
import {
  Streckengeschaeft, StreckengeschaeftFormData, StreckengeschaeftSchema, VorgangsTyp, StreckenStatus, getVorgangsTypLabel, getStatusLabel, formatCurrency, calculateGewinn, calculateGewinnmarge, calculateDeckungsbeitrag} from '../../types/streckengeschaeft';;
const { _TabPane,} = Tabs;;
const { _Option,} = Select;;
const { _Title, _Text,} = Typography;;
interface StreckengeschaeftFormProps {
  initialData?: Streckengeschaeft | null;
  onSubmit: (data: _StreckengeschaeftFormData) => void;
  onCancel: () => void;
  loading?: boolean;
}

export const StreckengeschaeftForm: React.FC<StreckengeschaeftFormProps> = ({
  initialData, onSubmit, onCancel, loading = false, }) => {;
const {
    control,
    handleSubmit,
    watch,
...
const { _ekPreis, _vkPreis, _menge, _frachtkosten, _ekMenge, _vkMenge,} = watchedValues;
...
const restwert = summeVk - summeEk - frachtkosten;,
  ...
  useEffect(() => {
    if (initialData) {;
const formData: unknown = { ...initialData };
