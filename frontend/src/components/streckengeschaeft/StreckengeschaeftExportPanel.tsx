...
import {
  Card, Typography, Button, Space, Divider, Row, Col, Select, Input, Checkbox, Modal, Form, message, Upload, Progress, Alert, Tabs, List, Avatar, Tag, Tooltip, Popconfirm, Switch, InputNumber, Radio, UploadFile} from 'antd';
import {
  DownloadOutlined, PrinterOutlined, MailOutlined, WhatsAppOutlined, FileExcelOutlined, FilePdfOutlined, FileTextOutlined, UploadOutlined, SettingOutlined, ReloadOutlined, CheckCircleOutlined, WarningOutlined, InfoCircleOutlined, CloudUploadOutlined, // ConvertOutlined} from '@ant-design/icons';
...
export const StreckengeschaeftExportPanel: React.FC<StreckengeschaeftExportPanelProps> = ({
  filter, selectedIds = [], onExportComplete, }) => {;
...
const handleWhatsApp = async () => {
    ...
    setLoading(true);
    try {;
const { _phoneNumber, message: whatsappMessage, _includeAttachment} = form.getFieldsValue();
...
const { _targetFormat, _quality, _password, _watermark,} = form.getFieldsValue();
...
const newJob: ConversionJob = {
        ...
        try {;
const blob = await StreckengeschaeftApi.convertFile(file.originFileObj, targetFormat, { quality, password, watermark, });
          ...
          setConversionJobs(prev =>, prev.map(job =>, job.id === jobId, ? { ...job, status: 'error' as const, error: error instanceof Error ? error.message : 'Unbekannter Fehler' }
                : job)
          );
...
const getPrinterStatusText = (status: string) => {
  ...
  return (
    <Card 
      title={
        <Space>, <DownloadOutlined />, <span>Export & Kommunikation</span>, </Space>, }
      extra={
            ...
            <Col span={12,}>
              <Form layout="vertical">
                <Form.Item label="Verfügbare Drucker">
                  ...
                  >
                    {printers.map(printer => (, <Option key={printer.id, } value={printer.id, }>
                        <Space>
      ...
      >
        <Form form={form,} layout="vertical">
          ...
          <List
            ...
            renderItem={job => (<List.Item>, <List.Item.Meta, avatar={
                    <Avatar icon={
