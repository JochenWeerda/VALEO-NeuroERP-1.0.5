...
import {
  Table, TableBody, TableCell, TableContainer, TableHead, TableRow, Paper, TablePagination, TableSortLabel, Chip, IconButton, Tooltip, Box, Typography, TextField, InputAdornment, FormControl, InputLabel, Select, MenuItem, Alert, CircularProgress, Card, CardContent} from '@mui/material';
import {
  Visibility as ViewIcon, Edit as EditIcon, Delete as DeleteIcon, Search as SearchIcon, FilterList as FilterIcon, Euro as EuroIcon, Person as PersonIcon, Assignment as AssignmentIcon, Info as InfoIcon, Warning as WarningIcon} from '@mui/icons-material';
...
const [invoiceResult, customerResult] = await Promise.all([, invoiceData.fetchData(),
        ...
        console.log('✅ Daten erfolgreich geladen:', {
          invoices: invoiceResult.data?.length || 0, customers: customerResult.data?.length || 0
        });
...
const renderRLSInfo = () => {
    ...
    return (<Card className="mb-4">, <CardContent>, <Typography variant="h6" className="flex items-center mb-2">, <InfoIcon className="mr-2" />, RLS-Compliance Status, </Typography>, <div className="grid grid-cols-2 gap-4 text-sm">, <div>, <strong>Bearbeitbar:</strong> {mcpTable.schema.rls.update ? '✅ Ja' : '❌ Nein'}
            </div>
            <div>
          ...
          {!mcpTable.schema.rls.update && (, <Alert severity="warning" className="mt-2">, <Typography variant="caption">, ⚠️ Rechnungen können nach dem Erstellen nicht mehr bearbeitet werden, </Typography>, </Alert>),}
        </CardContent>
      </Card>
    );
  };
  ...
  if (mcpTable.isLoading || isLoading) {
    return (<Box className="flex justify-center items-center h-64">, <Box className="text-center">, <CircularProgress className="mb-4" />, <Typography>Lade Schema-Informationen und Daten...</Typography>, </Box>, </Box>);,
  }

  // Error-State
  if (error || mcpTable.error) {
    return (<Alert severity="error" className="mb-4">, {error || mcpTable.error?.message || 'Unbekannter Fehler', }
      </Alert>);
  }

  // Schema nicht verfügbar
  if (!mcpTable.schema) {
    return (<Alert severity="warning">, Schema-Informationen konnten nicht geladen werden, </Alert>);,
  }
                ...
                .map((invoice) => (<TableRow key={invoice.id, } hover>
                    ...
                    <TableCell align="center">
                      <Box className="flex justify-center space-x-1">
                        {onView && (<Tooltip title="Anzeigen">, <IconButton, size="small", onClick={() => onView(invoice),}
                            >
                              <ViewIcon />
                            </IconButton>
                          </Tooltip>
                        ...
                        {onEdit && mcpTable.schema.rls.update && (<Tooltip title="Bearbeiten">, <IconButton, size="small", onClick={() => onEdit(invoice),}
                            >
                              <EditIcon />
                            </IconButton>
                          </Tooltip>
                        ...
                        {onDelete && mcpTable.schema.rls.delete && (<Tooltip title="Löschen">, <IconButton, size="small", onClick={() => onDelete(invoice),}
                            >
                              <DeleteIcon />
                            </IconButton>
                          </Tooltip>
...
export default InvoiceTable_MCP; 