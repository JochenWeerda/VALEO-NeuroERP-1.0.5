...
import {
  Card, CardContent, Stepper, Step, StepLabel, StepContent, Typography, Box, Chip, IconButton, Tooltip, LinearProgress, CircularProgress} from '@mui/material';
import {
  ArrowBack as ArrowBackIcon, ArrowForward as ArrowForwardIcon, Check as CheckIcon, Warning as WarningIcon, Info as InfoIcon, SaveAlt as AutoSaveIcon, Delete as DeleteIcon, Edit as EditIcon, Visibility as ViewIcon, Download as ExportIcon, Upload as ImportIcon} from '@mui/icons-material';
...
import { 
  StandardTextField, StandardSelectField, StandardButton, FormActions, FormMessage} from './FormStandardization';
import { UI_LABELS, StatusChip, StandardMessage} from '../ui/UIStandardization';

// Types und Services
import {
  FormTab, FormTimeline, TimelineStep, Belegfolge, WorkflowStep, FormLayout, StandardizedFormConfig} from '../../types/forms';
...
interface ModernERPFormWithDBProps {
  ...
  onExport?: (filters?: unknown) => Promise<void>;
...
export const ModernERPFormWithDB: React.FC<ModernERPFormWithDBProps> = ({
  config, initialData, mode, recordId, onSave, onCancel, onDelete, onExport, onImport, className = '', showActions = true, autoLoad = true, }) => {
  // Form State Management,;
const [formState, setFormState] = useState<FormState>({
    currentTab: 0, currentStep: 0, isSubmitting: false, isLoading: false, autoSaveStatus: 'idle', validationErrors: [], data: initialData || {}, mode
  });
...
const response = await formDataService.getFormDataById(config, id.toString());,
      ...
      if (response.success && response.data) {
        reset(response.data);,
        setFormState(prev => ({ 
          ...prev, data: response.data, isLoading: false 
        }));
    ...
    <Card className={`modern-erp-form ${className, }`}>
      <CardContent>
        ...
        <Box display="flex" justifyContent="space-between" alignItems="center" mb={3, }>
          ...
          {showActions && (, <Box display="flex" gap={1, }>
              {formState.mode === 'view' && (, <StandardButton, variant="outlined", startIcon={<EditIcon />, }
              ...
              {formState.mode !== 'view' && (<StandardButton, variant="outlined", startIcon={<ViewIcon />, }
...
variant="outlined"
              ...
              {formState.mode === 'edit' && recordId && (<StandardButton, variant="outlined", color="error", startIcon={<DeleteIcon />, }
...
variant="determinate" 
        ...
        {config.features?.autoSave && formState.autoSaveStatus !== 'idle' && (<StandardMessage, type={formState.autoSaveStatus === 'saved' ? 'success' : 'error'}
            title="Auto-Save Status:"
            message={
              formState.autoSaveStatus === 'saving', ? 'Auto-Save läuft...', : formState.autoSaveStatus === 'saved' 
                ? 'Auto-Save erfolgreich', : 'Auto-Save fehlgeschlagen'
            }
          />)}

        {/* Form Content */,}
        <form onSubmit={handleSubmit(onSubmit),}>
          {config.layout?.type === 'tabs' && config.layout.tabs ? (<Stepper activeStep={formState.currentTab, } orientation="vertical">
              {config.layout.tabs.map((tab: FormTab, index: number) => (<Step key={tab.id}>
                  <StepLabel>{tab.label, }</StepLabel>
                  <StepContent>
                    <Box sx={{ mt: 2, mb: 2 }}>
                      {tab.fields?.map((fieldName) => (<StandardTextField, key={fieldName.toString(),}
...
variant="outlined"
          ...
          {formState.mode !== 'view' && (<FormActions, onSave={handleSubmit(onSubmit),}
              onCancel={onCancel,}
