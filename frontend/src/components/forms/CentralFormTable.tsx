...
import {
  Table, TableBody, TableCell, TableContainer, TableHead, TableRow, Paper, Chip, IconButton, Tooltip, TextField, Select, MenuItem, FormControl, InputLabel, Box, Typography, Card, CardContent, Grid, Button, Dialog, DialogTitle, DialogContent, DialogActions, Alert, Badge} from '@mui/material';
import {
  Search as SearchIcon, FilterList as FilterIcon, Sort as SortIcon, Edit as EditIcon, Visibility as ViewIcon, Security as SecurityIcon, History as HistoryIcon, Download as DownloadIcon, Upload as UploadIcon, Refresh as RefreshIcon} from '@mui/icons-material';
...
const [filterState, setFilterState] = useState<FilterState>({
    searchTerm: '', module: '', status: '', complexity: '', category: ''
  });;
...
const searchTerm = filterState.searchTerm.toLowerCase();,
      filtered = filtered.filter(entry =>, entry.title.toLowerCase().includes(searchTerm) ||,
...
const handleSort = (field: keyof FormTableEntry) => {
    setSortState(prev => ({
      field, direction: prev.field === field && prev.direction === 'asc' ? 'desc' : 'asc'
    }));
  };;
    ...
    <div className={`central-form-table ${className, }`}>
      ...
      <Card className="mb-6">
        <CardContent>
          ...
          {statistics && (, <Grid container spacing={2, } className="mt-4">
...
variant="outlined"
      ...
      >
        ...
        <DialogContent>
          {selectedEntry && (<Grid container spacing={2, }>
      ...
      >
        ...
        <DialogContent>
          {selectedEntry && (<Alert severity="info" className="mb-4">, Hier können die Berechtigungen für das Formular angepasst werden., </Alert>),}
        </DialogContent>
        <DialogActions>
