...
import {
  Card, CardContent, Typography, Box, Tabs, Tab, Alert, CircularProgress, Chip, Divider, Grid, FormControl, InputLabel, Select, MenuItem, FormHelperText} from '@mui/material';
...
export const ModernERPForm: React.FC<ModernERPFormProps> = ({
  config, initialData = {}, onSave, onCancel, mode = 'create', recordId
}) => {;
...
let fieldSchema = z.string();,
      ...
      if (field.validation?.min) {
        fieldSchema = fieldSchema.refine(val => typeof val === 'string' && val.length >= field.validation!.min!, { message: `${field.label} muss mindestens ${field.validation!.min, } Zeichen lang sein` });
      }

      // Maximale Länge
      if (field.validation?.max) {
        fieldSchema = fieldSchema.refine(val => typeof val === 'string' && val.length <= field.validation!.max!, { message: `${field.label} darf maximal ${field.validation!.max, } Zeichen lang sein` });
      }
...
const issues = await formSecurityManager.validateFormData(config.security, initialData, {
          formId: config.id || 'unknown', userId: 'current-user', sessionId: 'current-session', ipAddress: '127.0.0.1', userAgent: navigator.userAgent, timestamp: new Date(),
...
const securityWarning = securityWarnings[field.name];,
    ...
    return (<Grid item xs={12, } sm={field.gridSize || 6, } key={field.name, }>
        {field.type === 'select' ? (, <StandardSelectField, name={field.name, }
            label={field.label, }
            options={field.options || [], }
            required={field.required, }
            disabled={mode === 'view', }
            helperText={
              fieldError?.message ||, (validationError ? validationError.join(', ') : '') ||
  ...
  return (
    <Card sx={{ maxWidth: 800, mx: 'auto', mt: 2 }}>
      <CardContent>
        ...
        {config.description && (, <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
            {config.description, }
          </Typography>)}

        {/* ✅ REFAKTORIERT: Sicherheitswarnungen mit StandardMessage */}
        {securityIssues.length > 0 && (<StandardMessage, type="error", title="Sicherheitsprobleme erkannt:"
            message={securityIssues.join(', '),}
          />
        )}

        {/* ✅ REFAKTORIERT: Sicherheitswarnungen mit StandardMessage */}
        {Object.keys(securityWarnings).length > 0 && (<StandardMessage, type="warning", title="Sicherheitswarnungen:"
