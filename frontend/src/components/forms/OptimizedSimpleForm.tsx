...
import {
  TextField, Button, Alert, Box, Typography, CircularProgress, FormControl, InputLabel, Select, MenuItem, FormHelperText, Checkbox, FormControlLabel, Snackbar, IconButton, Tooltip, Chip, Divider, Paper, Fade, Zoom} from '@mui/material';
import {
  Save as SaveIcon, Cancel as CancelIcon, KeyboardArrowRight as ArrowIcon, QrCodeScanner as BarcodeIcon, AutoFixHigh as AutoCompleteIcon, ErrorOutline as ErrorIcon, CheckCircle as SuccessIcon, Warning as WarningIcon} from '@mui/icons-material';
...
interface OptimizedSimpleFormProps {
  fields: FormField[];
  onSubmit: (data: _FieldValues) => void | Promise<void>;
...
type={field.type, }
            ...
            InputProps={{
              startAdornment: field.icon, autoComplete: field.autoComplete ? 'on' : 'off'
            }}
          />);
...
type="date"
  ...
  return (<Controller
      ...
      render={({ field: { onChange, value, ref } }) => (
        <Box className="mb-4">
          {renderField(),}
          {field.helpText && !errorMessage && (<Typography variant="caption" color="textSecondary" className="mt-1">, {field.helpText, }
            </Typography>)}
        </Box>
      )}
    />
...
export const OptimizedSimpleForm: React.FC<OptimizedSimpleFormProps> = React.memo(({
  fields, onSubmit, defaultValues = {}, loading = false, error, submitText = 'Speichern', cancelText = 'Abbrechen', onCancel, className = '', showCancelButton = true, disabled = false, autoSave = true, autoSaveInterval = 30000, // 30 Sekunden
  showProgress = true, keyboardShortcuts = true, barcodeScanner = false, theme: themeMode = 'auto', size = 'medium', layout = 'vertical', validationSchema, onFieldChange, onAutoSave, onBarcodeScan, onKeyboardShortcut
}) => {;
...
const form = useForm({
    defaultValues, resolver: validationSchema ? zodResolver(validationSchema) : undefined,
...
const group = field.group || 'allgemein';, if (!groups[group]) {
        groups[group] = [];,
      }
...
const filledFields = Object.keys(formValues).filter(key =>, formValues[key] !== undefined && formValues[key] !== '').length;,
...
className={`p-6 ${className, }`}
      sx={{
        borderRadius: 2, backgroundColor: theme.palette.background.paper, }}
    >
      {/* Progress Bar */, }
      {showProgress && (, <Box className="mb-6">, <Box display="flex" justifyContent="space-between" alignItems="center" mb={1, }>
...
className="h-full bg-blue-500 transition-all duration-300"
              sx={{ 
                backgroundColor: theme.palette.primary.main, width: `${progress}%`
              }}
            />
          </Box>
        </Box>)}
...
className="mb-4"
            icon={<ErrorIcon />, }
            action={
              <IconButton, color="inherit", size="small", onClick={() => {/* Error dismiss */,}}
              >
...
className={`space-y-6 ${layout === 'grid' ? 'grid grid-cols-1 md:grid-cols-2 gap-6' : ''}`}
      >
        ...
        {Object.entries(groupedFields).map(([groupName, groupFields]) => (<Box key={groupName, }>
            {groupName !== 'allgemein' && (, <Typography variant="h6" className="mb-4 text-gray-800 border-b pb-2">, {groupName.charAt(0).toUpperCase() + groupName.slice(1),}
              </Typography>
            )}
...
className="flex-1 sm:flex-none"
        ...
        {keyboardShortcuts && (<Box className="mt-4 p-3 bg-gray-50 rounded-lg">, <Typography variant="caption" color="textSecondary">, <strong>Tastenkürzel:</strong> Strg+S (Speichern), Esc (Abbrechen), Tab (Navigation)
            </Typography>,
          </Box>,
        ),}
      </form>
