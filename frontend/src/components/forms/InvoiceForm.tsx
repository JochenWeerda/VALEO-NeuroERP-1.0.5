...
import { 
  Card, CardContent, CardHeader, Box, Typography, Divider, Chip, InputAdornment} from '@mui/material';
import { 
  Assignment as AssignmentIcon, Person as PersonIcon, Warning as WarningIcon, Info as InfoIcon} from '@mui/icons-material';
import { z ,} from 'zod';
...
interface InvoiceFormProps {
  ...
  onSubmit: (data: _InvoiceFormData) => Promise<void>;
  onCancel?: () => void;
  isLoading?: boolean;
}
...
const mockCustomers = [,
        ...
        setSchemaInfo({
          rls: {
            select: true, insert: true, update: false, // Business rule: Invoices cannot be updated
  ...
  return (
    <Card className="max-w-2xl mx-auto shadow-lg">
      ...
      <CardContent>
        ...
        {error && (<StandardMessage, type="error", message={error, }
          />)}

        {/* ✅ REFAKTORIERT: Formular-Validierungsfehler mit StandardMessage */}
        {Object.keys(errors).length > 0 && (<StandardMessage, type="warning", title="Validierungsfehler:"
        ...
        <form onSubmit={handleSubmit(handleFormSubmit),} className="space-y-4">
          ...
          <StandardSelectField
            ...
            options={customers.map(customer => ({
              value: customer.id, label: `${customer.name} (${customer.email, })`
            }))}
            required={true,}
