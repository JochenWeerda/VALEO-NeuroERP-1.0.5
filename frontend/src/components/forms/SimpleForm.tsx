...
import {
  Alert, Box, Typography, CircularProgress, FormControl, InputLabel, Select, MenuItem, FormHelperText, Checkbox, FormControlLabel} from '@mui/material';
...
interface SimpleFormProps {
  fields: FormField[];
  onSubmit: (data: _any) => void | Promise<void>;
...
export const SimpleForm: React.FC<SimpleFormProps> = ({
  fields, onSubmit, defaultValues = {}, loading = false, error, submitText = UI_LABELS.ACTIONS.SAVE, // ✅ REFAKTORIERT: Standardisiertes Label
  cancelText = UI_LABELS.ACTIONS.CANCEL, // ✅ REFAKTORIERT: Standardisiertes Label
  onCancel, className = '', showCancelButton = true, disabled = false
}) => {;
const {
    control,
    handleSubmit,
...
const errorMessage = fieldError?.message as string;,
    ...
    switch (field.type) {
      ...
      case 'email':
      case 'password':
        return (<StandardTextField, name={field.name, }
            label={field.label, };
type={field.type, }
...
className="mb-4"
  ...
  return (
    <Box className={`space-y-6 ${className, }`}>
      ...
      {error && (, <StandardMessage, type="error", message={error, }
        />)}
