...
interface WaWiFormProps {
  ...
  onSave: (data: _any) => Promise<void>;
  onCancel: () => void;;
className?: string;
}

export const WaWiForm: React.FC<WaWiFormProps> = ({
  formId, initialData, mode, onSave, onCancel, className = '', }) => {;
const formRegistry = ExtendedFormRegistryService.getInstance();,;
const formConfig = formRegistry.getForm(formId);,

  if (!formConfig) {
    return (<div className="p-4 bg-red-50 border border-red-200 rounded-lg">, <h3 className="text-red-800 font-semibold">Formular nicht gefunden</h3>, <p className="text-red-600">Das Formular "{formId, }" ist nicht in der Registry registriert.</p>
      </div>);
  }
...
const wawiForms = formRegistry.getFormsByModule('warenwirtschaft');,
  ...
  return (<div className="w-full">, <div className="mb-6">, <Typography variant="h4" component="h2" className="text-2xl font-bold text-gray-900 mb-2">, Warenwirtschaft - {formRegistry.getForm(selectedForm)?.metadata.name || 'Formular',}
        </Typography>
...
className={`p-3 rounded border cursor-pointer transition-colors ${
                selectedForm === form.id, ? 'bg-blue-50 border-blue-200 text-blue-800', : 'bg-white border-gray-200 hover:bg-gray-50'
              }`}
            >
