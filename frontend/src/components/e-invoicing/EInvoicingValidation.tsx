...
import {
  Card, CardContent, Typography, List, ListItem, ListItemIcon, ListItemText, Alert, Box, Chip, Button, Collapse} from '@mui/material';
import {
  CheckCircle, Error, Warning, ExpandMore, ExpandLess, Refresh} from '@mui/icons-material';
...
export const EInvoicingValidation: React.FC<EInvoicingValidationProps> = ({
  validationResult, onRevalidate, isLoading = false, }) => {;
    ...
    <Card className="w-full">
      <CardContent className="p-4">
        ...
        <div className="flex justify-between items-center mb-4">
          ...
          <div className="flex items-center space-x-2">
            {onRevalidate && (, <Button, size="small", variant="outlined", onClick={handleRevalidate, }
                disabled={isLoading, }
                startIcon={<Refresh />, }
              >
                Neu validieren
...
variant="text"
        ...
        {/* Gesamtstatus */,}
        {validationResult.isValid ? (<Alert severity="success" className="mb-4">, <Typography variant="body2">, Die Rechnung ist gültig und entspricht allen Anforderungen., </Typography>, </Alert>) : (
          <Alert severity="error" className="mb-4">, <Typography variant="body2">, Die Rechnung enthält Fehler und muss korrigiert werden., </Typography>, </Alert>),}

        <Collapse in={expanded,}>
          {/* Fehler */,}
          {validationResult.errors.length > 0 && (<Box className="mb-4">, <Typography variant="subtitle1" className="mb-2 text-red-600 font-semibold">, Fehler ({validationResult.errors.length, })
              </Typography>
...
className: 'text-red-700'
          ...
          {/* Warnungen */,}
          {validationResult.warnings.length > 0 && (<Box className="mb-4">, <Typography variant="subtitle1" className="mb-2 text-orange-600 font-semibold">, Warnungen ({validationResult.warnings.length, })
              </Typography>
...
className: 'text-orange-700'
          ...
          {validationResult.errors.length === 0 && validationResult.warnings.length === 0 && (<Box>, <Typography variant="subtitle1" className="mb-2 text-green-600 font-semibold">, Alle Prüfungen erfolgreich, </Typography>, <List dense className="bg-green-50 rounded-lg">, <ListItem className="py-1">, <ListItemIcon className="min-w-0 mr-2">, {getValidationIcon('success'),}
                  </ListItemIcon>
                  <ListItemText
