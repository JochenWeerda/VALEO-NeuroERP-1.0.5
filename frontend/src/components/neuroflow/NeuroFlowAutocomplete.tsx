...
import {
  TextField, Autocomplete as MuiAutocomplete, CircularProgress, Chip, Box, Typography, Popper, Paper, ListItem, ListItemText, ListItemIcon, Divider, Alert, Tooltip} from '@mui/material';
import {
  Search as SearchIcon, Person as PersonIcon, Business as BusinessIcon, LocationOn as LocationIcon, Phone as PhoneIcon, Email as EmailIcon, AccountBalance as BankIcon, Science as ScienceIcon, LocalShipping as ShippingIcon, Payment as PaymentIcon, Description as DescriptionIcon, CheckCircle as CheckCircleIcon, Warning as WarningIcon, Info as InfoIcon} from '@mui/icons-material';
...
const CustomPopper = styled(Popper)(({ theme, }) => ({
  '& .MuiAutocomplete-paper': {
    borderRadius: theme.shape.borderRadius * 1.5, boxShadow: theme.shadows[8], border: `1px solid ${theme.palette.divider}`, maxHeight: 400, }, }));
...
interface NeuroFlowAutocompleteProps {
  ...
  placeholder?: string;
  value: string;
  onChange: (value: _string, option?: _AutocompleteOption) => void;
...
export const NeuroFlowAutocomplete: React.FC<NeuroFlowAutocompleteProps> = ({
  label, placeholder, value, onChange, onSelect, type, required = false, disabled = false, error = false, helperText, fullWidth = true, size = 'medium', variant = 'outlined', showChips = false, multiple = false, maxSuggestions = 10, minChars = 2, debounceMs = 300, apiEndpoint, customOptions = [], onLoadOptions, renderOption, getOptionLabel, filterOptions, }) => {;
...
const scoredOptions = allOptions.map(option => ({
          ...option, score: fuzzySearch(query, option.label),
...
const defaultRenderOption = (option: AutocompleteOption) => (
    <ListItem dense>
      ...
      <ListItemText
        ...
        secondary={
          option.metadata && (<Typography variant="caption" color="text.secondary">, {option.metadata.description || option.metadata.category || option.category, }
            </Typography>)
        }
      />
    </ListItem>
