...
import { 
  Badge, IconButton, Menu, MenuItem, Typography, Box, Chip, Divider} from '@mui/material';
import { 
  Notifications as NotificationsIcon, CheckCircle as CheckCircleIcon, Info as InfoIcon, Warning as WarningIcon, Error as ErrorIcon} from '@mui/icons-material';
      ...
      >
        ...
        <Box sx={{ p: 2, borderBottom: 1, borderColor: 'divider' }}>
          <Box display="flex" alignItems="center" justifyContent="space-between" mb={1,}>
            ...
            {unreadCount > 0 && (<Chip, label={`${unreadCount, } Ungelesen`}
                size="small"
                color="primary"
              />)}
          </Box>
          ...
          <Box display="flex" gap={0.5,}>
            {(['all', 'unread', 'ai', 'system', 'business'] as const).map(filter => (, <Chip, key={filter, }
...
variant={activeFilter === filter ? 'filled' : 'outlined'}
        ...
        <Box sx={{ maxHeight: 256, overflowY: 'auto' }}>
          {filteredNotifications.length === 0 ? (<Box sx={{ p: 2, textAlign: 'center', color: 'text.secondary' }}>
            ...
            </Box>) : (filteredNotifications.map((notification, index) => (
                    ...
                    <Box flex={1,} minWidth={0,}>
                      <Box display="flex" alignItems="center" justifyContent="space-between" mb={0.5,}>
                        ...
                        <Box display="flex" alignItems="center" gap={1,}>
                          {notification.source && (<Chip, label={notification.source, }
                              size="small"
                              sx={{ fontSize: '0.75rem' }}
                            />)}
                          <Chip
