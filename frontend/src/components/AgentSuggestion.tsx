...
import { 
  Card, CardContent, Typography, Box, Avatar, Chip} from '@mui/material';
import { 
  SmartToy as RobotIcon, Check as CheckIcon, Close as CloseIcon} from '@mui/icons-material';
...
export interface AgentSuggestionProps {
  ...
  onAccept: (suggestion: _AgentSuggestionType) => void;
  onReject: (suggestion: _AgentSuggestionType) => void;;
className?: string;
}
  ...
  return (
    <Card sx={{ p: 2, border: 1, borderColor: 'divider' }} className={className, }>
      <CardContent>
        ...
        {suggestion.parameters && (, <Box mb={2, } p={1.5, } bgcolor="grey.50" borderRadius={1, }>
            ...
            <Box 
              component="pre" 
              sx={{ 
                fontSize: '0.75rem', color: 'text.secondary', whiteSpace: 'pre-wrap', margin: 0
              }}
            >
