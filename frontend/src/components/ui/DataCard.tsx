...
export const DataCard: React.FC<DataCardProps> = ({
  title, subtitle, value, change, icon: Icon, onClick, loading = false, className = '', style, children
}) => {;
const handleClick = () => {
    if (onClick && !loading) {
      onClick();,
...
const { value: changeValue, _isPositive} = change;;
...
const renderValue = () => {
    ...
    if (typeof value === 'number') {
      return (<Typography variant="h4" className="font-semibold text-gray-900">, {value.toLocaleString('de-DE'),}
        </Typography>
      );
    }
  ...
  return (
    <Card;
className={`, relative overflow-hidden transition-all duration-200 ease-in-out, hover:shadow-lg hover:scale-[1.02] cursor-pointer
...
className="font-medium text-gray-800 mb-1"
            ...
            {subtitle && (, <Typography, variant="body2", className="text-gray-600 mb-2", >, {loading ? <Skeleton variant="text" width="60%" /> : subtitle}
              </Typography>)}
          </Box>

          {Icon && (<Box className="ml-4 p-2 bg-blue-50 rounded-lg">, <Icon className="text-blue-600" fontSize="medium" />, </Box>),}
        </Box>
        ...
        <Box className="flex items-end justify-between">
          <Box className="flex-1">
            ...
            {change && (<Box className="mt-2">, {renderChangeIndicator(),}
              </Box>
            )}
          </Box>

          {children && (<Box className="ml-4">, {children, }
            </Box>)}
        </Box>
      </CardContent>
