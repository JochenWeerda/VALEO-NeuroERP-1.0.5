...
import {
  Table, TableBody, TableCell, TableContainer, TableHead, TableRow, Paper, Typography, Box, Chip, IconButton, Button, Dialog, DialogTitle, DialogContent, DialogActions, TextField, Select, MenuItem, FormControl, InputLabel, Alert, Tooltip, Badge, Switch, FormControlLabel, Accordion, AccordionSummary, AccordionDetails, List, ListItem, ListItemText, ListItemSecondaryAction, Divider, Card, CardContent, Grid, Tabs, Tab} from '@mui/material';
import {
  Edit as EditIcon, History as HistoryIcon, Security as SecurityIcon, Visibility as ViewIcon, Add as AddIcon, Delete as DeleteIcon, CheckCircle as ApprovedIcon, Cancel as RejectedIcon, Pending as PendingIcon, ExpandMore as ExpandMoreIcon, Settings as SettingsIcon, Person as PersonIcon, Business as BusinessIcon, Inventory as InventoryIcon, AttachMoney as FinanceIcon, LocalShipping as SupplierIcon, ShoppingCart as OrderIcon, Assessment as ReportingIcon, Settings as SettingsIcon2} from '@mui/icons-material';
import { useTheme ,} from '@mui/material/styles';
import { formRegistryService ,} from '../../services/FormRegistryService';
import {
  FormID, FormVersion, FormStatus, FormPermission, RolePermissions, FormConfig, FormVersionHistory, FormChangeRequest, FormChange, FORM_MODULES, FORM_STATUSES, FORM_PERMISSIONS} from '../../types/forms';;
...
export const FormRegistryTable: React.FC<FormRegistryTableProps> = ({
  currentUserRole, onFormEdit, onVersionHistory, onPermissionsEdit, onChangeRequest, }) => {;
...
variant="outlined"
                  ...
                  <TableCell>
                    <Box display="flex" gap={0.5,}>
                      {hasPermission(form.id, FORM_PERMISSIONS.READ) && (<Tooltip title="Anzeigen">, <IconButton size="small" onClick={() => onFormEdit?.(form.id),}>
                            <ViewIcon />
                          </IconButton>
                        </Tooltip>
                      )}
                      
                      {hasPermission(form.id, FORM_PERMISSIONS.WRITE) && (<Tooltip title="Bearbeiten">, <IconButton size="small" onClick={() => onFormEdit?.(form.id),}>
                            <EditIcon />
                          </IconButton>
                        </Tooltip>
                      )}
                      ...
                      {hasPermission(form.id, FORM_PERMISSIONS.ADMIN) && (<Tooltip title="Berechtigungen">, <IconButton, size="small", onClick={() => handlePermissionsOpen(form),}
                          >
                            <SecurityIcon />
                          </IconButton>
                        </Tooltip>
...
variant="contained"
      ...
      >
        ...
        <DialogContent>
          {selectedForm && (<PermissionsEditor, formId={selectedForm.id, }
...
interface PermissionsEditorProps {
  ...
  onSave: (formId: _FormID, permissions: _RolePermissions) => void;
};
const PermissionsEditor: React.FC<PermissionsEditorProps> = ({
  formId, currentPermissions, onSave, }) => {;
        ...
        <Table size="small">
          <TableHead>
            <TableRow>
              ...
              {permissionTypes.map(permission => (, <TableCell key={permission, } align="center">
                  {permission, }
                </TableCell>))}
            </TableRow>
          </TableHead>
          <TableBody>
            {roles.map(role => (, <TableRow key={role, }>
                <TableCell>
  ...
  return (<Box>
      ...
      {versionHistory.map((version, index) => (<Accordion key={version.version, }>
          ...
          <AccordionDetails>
            ...
            {version.approvedBy && (<Typography variant="caption" color="textSecondary">, Genehmigt von: {version.approvedBy}
              </Typography>)}
          </AccordionDetails>
        </Accordion>
      ))}
