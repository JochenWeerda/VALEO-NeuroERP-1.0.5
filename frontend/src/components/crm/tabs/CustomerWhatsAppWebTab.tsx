...
import {
  Box, Card, CardContent, Typography, Button, TextField, Grid, Chip, Alert, Dialog, DialogTitle, DialogContent, DialogActions, IconButton, List, ListItem, ListItemText, ListItemSecondaryAction, Divider, Paper} from '@mui/material';
import {
  WhatsApp as WhatsAppIcon, Send as SendIcon, QrCode as QrCodeIcon, History as HistoryIcon, Settings as SettingsIcon, Refresh as RefreshIcon, Delete as DeleteIcon, Edit as EditIcon, Add as AddIcon} from '@mui/icons-material';;
...
const whatsappUrl = `https://web.whatsapp.com/send?phone=${phoneNumber}&text=${encodedMessage,}`;
      ...
      setTimeout(() => {
        setMessageHistory(prev =>, prev.map(msg =>, msg.id === newMessage.id, ? { ...msg, status: 'sent' as const }
              : msg)
        );
      }, 2000);
      ...
      setMessageHistory(prev =>, prev.map(msg =>, msg.id === newMessage.id, ? { ...msg, status: 'failed' as const }
            : msg)
      );
    }
  };;
...
variant="outlined"
      ...
      {/* Verbindungsstatus */,}
      {!isConnected && (<Alert severity="warning" className="mb-4">, <Typography variant="body2">, WhatsApp Web ist nicht verbunden. Scannen Sie den QR-Code, um zu beginnen., </Typography>, <Button, variant="contained", size="small", onClick={handleConnectWhatsApp, };
className="mt-2"
          >
            QR-Code scannen
          </Button>
...
variant="outlined"
          ...
          <Card className="mt-4">
            <CardContent>
              ...
              <List>
                {messageHistory.length === 0 ? (<ListItem>, <ListItemText, primary="Keine Nachrichten gesendet", secondary="Hier werden Ihre gesendeten WhatsApp-Nachrichten angezeigt", />, </ListItem>) : (
                  messageHistory.map((msg) => (<React.Fragment key={msg.id, }>
                      <ListItem>
                        <ListItemText
                          primary={
                            <Box className="flex items-center space-x-2">, <span>{msg.customerName || msg.phoneNumber, }</span>
                              <Chip
                    ...
                    <Box className="flex justify-between items-start">
                      <Box className="flex-1">
                        ...
                        <Typography variant="body2" color="textSecondary" className="mt-1">
                          {template.message.length > 60, ? `${template.message.substring(0, 60),}...` 
                            : template.message
                          }
                        </Typography>
                        <Chip
