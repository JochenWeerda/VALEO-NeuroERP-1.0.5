...
import {
  Box, Typography, Card, CardContent, List, ListItem, ListItemText, ListItemAvatar, Avatar, Chip, Button, TextField, Dialog, DialogTitle, DialogContent, DialogActions, IconButton, Tooltip, Divider, Grid} from '@mui/material';
import {
  WhatsApp as WhatsAppIcon, Send as SendIcon, Reply as ReplyIcon, Forward as ForwardIcon, Delete as DeleteIcon, Archive as ArchiveIcon, Search as SearchIcon, FilterList as FilterIcon} from '@mui/icons-material';
...
interface CustomerWhatsAppHistoryTabProps {
  ...
  onSubTabChange: (subTab: _CRMSubTab) => void;
};
const CustomerWhatsAppHistoryTab: React.FC<CustomerWhatsAppHistoryTabProps> = ({
  customer, currentSubTab, onSubTabChange, }) => {;
...
const response = await fetch('/api/whatsapp/send', {
          ...
          body: JSON.stringify({
            to: selectedMessage.from, content: replyText, customerId: customer.id, originalMessageId: selectedMessage.id, type: 'reply'
          })
        });
...
variant="outlined"
          ...
          {filteredMessages.length === 0 && (<Box className="text-center py-8">, <WhatsAppIcon className="text-gray-400 text-4xl mb-2" />, <Typography variant="body2" color="textSecondary">, Keine WhatsApp-Nachrichten gefunden, </Typography>, </Box>),}
        </CardContent>
      </Card>
      ...
      >
        ...
        <DialogContent>
          <Box className="space-y-4">
            {/* Original Message */,}
            {selectedMessage && (<Card variant="outlined">, <CardContent className="p-3">, <Typography variant="caption" className="text-gray-500">, Original Nachricht:
                  </Typography>, <Typography variant="body2" className="mt-1">, {selectedMessage.content, }
                  </Typography>
                </CardContent>
              </Card>)}
