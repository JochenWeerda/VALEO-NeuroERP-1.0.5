...
import { 
  Security as SecurityIcon, CheckCircle as CheckCircleIcon, Warning as WarningIcon, Error as ErrorIcon, Info as InfoIcon} from '@mui/icons-material';
...
const TrustIndicator: React.FC<TrustIndicatorProps> = ({
  title, description, level, value, unit, trend, lastUpdated, details, className, confidence, }) => {
...
className={`trust-indicator ${className || '', }`}
      sx={{
        p: 2, height: '100%', display: 'flex', flexDirection: 'column', border: 2, borderColor: `${levelStatus}.main`, backgroundColor: `${levelStatus, }.50`, transition: 'all 0.3s ease', '&:hover': {
    ...
    >
      <Box display="flex" alignItems="center" mb={1,}>
        ...
        {title && (<Typography, variant="h6", component="h3", sx={{ 
              color: `${levelStatus}.main`, fontWeight: 600, flex: 1
            }}
          >
            {title, }
          </Typography>)}
...
variant="h4" 
            component="span"
            sx={{ 
              color: `${levelStatus}.main`, fontWeight: 700, mr: 0.5
            }}
          >
            {value, }
          </Typography>
          {unit && (, <Typography, variant="body2", sx={{ color: 'text.secondary' }}
            >
              {unit, }
            </Typography>)}
          {trend && (<Box ml={1, }>
      ...
      {details && details.length > 0 && (<Box mt={1, }>
          {details.map((detail, index) => (<Typography, key={index, };
variant="caption" 
              sx={{ 
                display: 'block', color: 'text.secondary', fontSize: '0.75rem'
              }}
            >
              • {detail, }
            </Typography>))}
      ...
      {lastUpdated && (<Typography, variant="caption", sx={{ 
            color: 'text.disabled', fontSize: '0.7rem', mt: 1, display: 'block'
          }}
        >
