...
export const FreightOrderForm: React.FC<FreightOrderFormProps> = ({
  header, customers, loading = false, error, onChangeHeader, onSubmit, onCancel, }) => {
  // Handler für Header-Änderungen,;
const handleHeaderChange = (field: keyof FreightOrderHeader, value: unknown) => {
          ...
          <Box>
            ...
            <Autocomplete
              ...
              renderTags={(value, getTagProps) =>,
                value.map((option, index) => (<Chip, label={`${option.name, } (${option.customerNumber, })`}
                    {...getTagProps({ index, })}
                    key={option.id,}
                  />
                ))
