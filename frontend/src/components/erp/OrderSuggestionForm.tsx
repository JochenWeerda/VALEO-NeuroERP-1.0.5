...
export const OrderSuggestionForm: React.FC<OrderSuggestionFormProps> = ({
  orderLines, suppliers, loading = false, error, onChangeOrderLines, onSubmit, onCancel, }) => {
...
const columns: GridColDef[] = [
    ...
    { field: 'vorschlag', headerName: 'Vorschlag', width: 100, editable: true, type: 'number' },
    {
      ...
      width: 150,
      editable: false,
      renderCell: (_params) => params.value?.name || '',
    },
...
const handleRowEdit = (params: unknown) => {;
...
const totalOrderValue = orderLines.reduce((sum, line) => sum + (line.bestellwert || 0), 0);
  ...
  return (
    <Card className="p-6 max-w-6xl mx-auto mt-6 shadow-md">
      ...
      {error && (, <Box className="mb-4">, <Typography color="error">{error, }</Typography>
        </Box>)}
      {loading ? (<Box className="flex justify-center items-center h-40">, <CircularProgress />, </Box>) : (
        <div className="space-y-6">, {/* Bestellzeilen */, }
          <div>
...
className="min-h-[400px]"
          ...
          <div className="bg-gray-50 p-4 rounded">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {orderLines.map((line) => (<Autocomplete, key={line.lagerNr, }
                  options={suppliers, }
