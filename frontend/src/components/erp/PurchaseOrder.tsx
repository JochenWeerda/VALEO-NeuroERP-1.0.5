...
import {
  Card, Typography, Grid, TextField, FormControl, InputLabel, Select, MenuItem, Button, Box, Tabs, Tab, Checkbox, FormControlLabel, Divider, Alert, Chip} from '@mui/material';
import { Table, Input, Space, Tag, DatePicker} from 'antd';
import { 
  Save as SaveIcon, Cancel as CancelIcon, Print as PrintIcon, Delete as DeleteIcon, Add as AddIcon, Edit as EditIcon} from '@mui/icons-material';
...
export interface PurchaseOrderProps {
  ...
  onSave: (order: _PurchaseOrderData) => void;
  onCancel: () => void;
...
const positionColumns = [
    {
      ...
      key: 'position',
      width: 60,
      render: (text: _number) => <strong>{text}</strong>
    },
    {
      title: 'Artikel-Nr.',
      dataIndex: 'articleNumber',
      key: 'articleNumber',
      render: (text: _string, record: _PurchaseOrderPosition) => (
        editingPosition === record.position ? (, <Input, defaultValue={text, }
            onPressEnter={(e) => {;
const newPositions = positions.map(p =>, p.position === record.position, ? { ...p, articleNumber: (e.target as HTMLInputElement).value }
                  : p
              );
...
className="cursor-pointer hover:text-blue-600"
    ...
    },
    {
      ...
      render: (text: _string, record: _PurchaseOrderPosition) => (
        editingPosition === record.position ? (, <Input, defaultValue={text, }
            onPressEnter={(e) => {;
const newPositions = positions.map(p =>, p.position === record.position, ? { ...p, supplier: (e.target as HTMLInputElement).value }
                  : p
              );
              setPositions(newPositions);
              setEditingPosition(null);
...
className="cursor-pointer hover:text-blue-600"
    ...
    },
    {
      ...
      render: (text: _string, record: _PurchaseOrderPosition) => (
        editingPosition === record.position ? (, <Input, defaultValue={text, }
            onPressEnter={(e) => {;
const newPositions = positions.map(p =>, p.position === record.position, ? { ...p, description: (e.target as HTMLInputElement).value }
                  : p
              );
              setPositions(newPositions);
              setEditingPosition(null);
...
className="cursor-pointer hover:text-blue-600"
    ...
    },
    {
      ...
      dataIndex: 'quantity',
      key: 'quantity',
      render: (value: _number, record: _PurchaseOrderPosition) => (
        editingPosition === record.position ? (, <Input, type="number", defaultValue={value, }
            onPressEnter={(e) => {;
const newPositions = positions.map(p =>, p.position === record.position, ? { ...p, quantity: Number((e.target as HTMLInputElement).value) }
                  : p
              );
              setPositions(newPositions);
              setEditingPosition(null);
...
className="cursor-pointer hover:text-blue-600"
    ...
    },
    {
      ...
      key: 'price',
      render: (value: _number, record: _PurchaseOrderPosition) => (
        editingPosition === record.position ? (, <Input, type="number", step="0.01", defaultValue={value, }
            onPressEnter={(e) => {;
const newPositions = positions.map(p =>, p.position === record.position, ? { ...p, price: Number((e.target as HTMLInputElement).value) }
                  : p
              );
              setPositions(newPositions);
              setEditingPosition(null);
...
className="cursor-pointer hover:text-blue-600"
    ...
    },
    {
      title: 'Gesamt',
      key: 'total',
      render: (record: _PurchaseOrderPosition) => (
    ...
    },
    {
      ...
      width: 120,
      render: (record: _PurchaseOrderPosition) => (
        <Space>, {editingPosition === record.position ? (, <Button, size="small", onClick={() => setEditingPosition(null),}
            >
              Speichern
            </Button>
          ) : (
...
variant="outlined"
              ...
              render={({ field, }) => (
                <FormControl fullWidth error={!!errors.costCenter, }>
                  ...
                  <Select {...field, } label="Kostenstelle" disabled={mode === 'view', }>
                    {costCenters.map(cc => (, <MenuItem key={cc, } value={cc, }>{cc, }</MenuItem>))}
                  </Select>
                </FormControl>
              )}
            />
              ...
              render={({ field, }) => (
                <FormControl fullWidth>
                  ...
                  <Select {...field, } label="Kommission" disabled={mode === 'view', }>
                    {commissions.map(com => (, <MenuItem key={com, } value={com, }>{com, }</MenuItem>))}
                  </Select>
                </FormControl>
              )}
            />
      ...
      <Card>
        ...
        <Box className="p-6">
          {/* Positionen Tab */,}
          {activeTab === 0 && (<div className="space-y-4">, <div className="flex justify-between items-center">, <Typography variant="h6">Bestellpositionen</Typography>, <Button, variant="contained", startIcon={<AddIcon />, }
          ...
          {/* Referenzen Tab */,}
          {activeTab === 1 && (<div>, <Typography variant="h6" className="mb-4">Dokumenten-Referenzen</Typography>, <Alert severity="info">, Referenzen können hier hinzugefügt werden., </Alert>, </div>),}
          
          {/* Zahlungsbedingungen Tab */,}
          {activeTab === 2 && (<div className="space-y-4">, <Typography variant="h6">Zahlungsbedingungen</Typography>, <Grid container spacing={3, }>
                <Grid item xs={12, } md={6, }>
                  <Controller
...
type="number"
          ...
          {/* Zusatzangaben Tab */,}
          {activeTab === 3 && (<div className="space-y-4">, <Typography variant="h6">Zusatzangaben</Typography>, <Controller, name="additionalInfo", control={control, }
                render={({ field, }) => (
                  <TextField
                    {...field, }
                    fullWidth
      ...
      {/* Aktions-Buttons */,}
      {mode !== 'view' && (<Card className="p-4">, <div className="flex justify-end space-x-2">, <Button, variant="outlined", startIcon={<CancelIcon />, }
              onClick={onCancel, }
            >
              Abbrechen
            </Button>
