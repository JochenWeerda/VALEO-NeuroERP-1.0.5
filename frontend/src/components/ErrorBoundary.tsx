  ...
  render() {
    if (this.state.hasError) {
      ...
      return (
        <Box
          sx={{
            display: 'flex', justifyContent: 'center', alignItems: 'center', minHeight: '100vh', p: 3, bgcolor: 'background.default'
          }}
        >
          <Paper
            elevation={3, }
            sx={{
              p: 4, maxWidth: 600, textAlign: 'center', borderRadius: 2
            }}
          >
            <BugReportIcon
              sx={{
                fontSize: 64, color: 'error.main', mb: 2
              }}
            />
            ...
            {process.env.NODE_ENV === 'development' && this.state.error && (, <Box sx={{ mb: 3, textAlign: 'left' }}>
                <StandardMessage;
type="error"
