...
import {
  Card, CardContent, Typography, Button, Dialog, DialogTitle, DialogContent, DialogActions, TextField, FormControl, InputLabel, Select, MenuItem, Alert, CircularProgress, Box, Grid, Chip} from '@mui/material';
import {
  Inventory as InventoryIcon, TrendingUp as TrendingUpIcon, Warning as WarningIcon, Refresh as RefreshIcon, Settings as SettingsIcon, Analytics as AnalyticsIcon, Speed as SpeedIcon, AttachMoney as MoneyIcon} from '@mui/icons-material';
...
const [optimizeParams, setOptimizeParams] = useState({
    min_confidence: 0.7, max_cost_impact: 1000, urgency_weight: 0.5
  });
...
const response = await fetch('/api/ai/inventory/optimize', {
        method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(optimizeParams)
      });
...
const matchesUrgency = filterUrgency === 'all' || suggestion.urgency_level === filterUrgency;, const matchesProduct = filterProduct === '' || suggestion.product_name.toLowerCase().includes(filterProduct.toLowerCase());,
...
className="min-w-48"
      ...
      <Dialog
        ...
        fullWidth
      >
        {selectedSuggestion && (<>, <DialogTitle>, <div className="flex items-center space-x-2">, <InventoryIcon />, <span>Vorschlag Details: {selectedSuggestion.product_name}</span>
              </div>
            </DialogTitle>
            <DialogContent>
...
type="number"
              ...
              onChange={(e) => setOptimizeParams({
                ...optimizeParams, min_confidence: parseFloat(e.target.value)
              })}
            />
            <TextField
              fullWidth
...
type="number"
              ...
              onChange={(e) => setOptimizeParams({
                ...optimizeParams, max_cost_impact: parseFloat(e.target.value)
              })}
            />
            <TextField
              fullWidth
...
type="number"
              ...
              onChange={(e) => setOptimizeParams({
                ...optimizeParams, urgency_weight: parseFloat(e.target.value)
              })}
            />
          </div>
        </DialogContent>
