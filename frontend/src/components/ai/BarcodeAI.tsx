...
import {
  Card, CardContent, Typography, Button, TextField, Chip, Box, Alert, CircularProgress, Grid, Paper, Divider} from '@mui/material';
import {
  QrCode as QrCodeIcon, AutoAwesome as AutoAwesomeIcon, TrendingUp as TrendingUpIcon, Category as CategoryIcon} from '@mui/icons-material';;
...
const response = await fetch('http://localhost:8000/api/ai/barcode/suggestions', {
        method: 'POST', headers: {
          'Content-Type': 'application/json', }, body: JSON.stringify({ product_name: productName }),
      });
    ...
    <Card className="w-full max-w-4xl mx-auto">
      <CardContent className="p-6">
        <Box className="flex items-center gap-3 mb-6">
          ...
          {aiHealth !== null && (, <Chip, label={aiHealth ? 'AI Service Online' : 'AI Service Offline'}
...
className="min-w-[140px]"
        ...
        {/* Vorschläge */,}
        {suggestions.length > 0 && (<Box>, <Typography variant="h6" className="mb-4">, AI-Barcode Vorschläge ({suggestions.length, })
            </Typography>
...
variant="outlined"
                    ...
                    {/* Ähnliche Produkte */,}
                    {suggestion.similar_products.length > 0 && (<Box className="mb-3">, <Typography variant="body2" className="text-gray-600 mb-1">, <strong>Ähnliche Produkte:</strong>
                        </Typography>, <Box className="flex gap-1 flex-wrap">, {suggestion.similar_products.map((product, index) => (<Chip, key={index, }
                              label={product, }
                              size="small";
variant="outlined"
                            />))}
...
variant="contained"
        ...
        {/* Info-Bereich */,}
        {suggestions.length === 0 && !loading && !error && (<Paper className="p-4 bg-blue-50">, <Typography variant="body2" className="text-blue-800">, <strong>💡 Tipp:</strong> Geben Sie einen Produktnamen ein, um intelligente 
              Barcode-Vorschläge basierend auf KI-Analyse zu erhalten. Das System berücksichtigt, Produktkategorien, Markttrends und ähnliche Produkte für optimale Empfehlungen., </Typography>, </Paper>),}
      </CardContent>
    </Card>
  );
}; 