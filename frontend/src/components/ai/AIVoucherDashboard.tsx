...
import {
  Card, CardContent, Typography, Button, Chip, Dialog, DialogTitle, DialogContent, DialogActions, TextField, FormControl, InputLabel, Select, MenuItem, Alert, CircularProgress, Box, Grid, IconButton, Tooltip, Table, TableBody, TableCell, TableContainer, TableHead, TableRow, Paper, LinearProgress, Slider} from '@mui/material';
import {
  LocalOffer as VoucherIcon, TrendingUp as TrendingUpIcon, People as PeopleIcon, AttachMoney as MoneyIcon, Refresh as RefreshIcon, Settings as SettingsIcon, Analytics as AnalyticsIcon, Psychology as PsychologyIcon, Assessment as AssessmentIcon} from '@mui/icons-material';
...
const [optimizationParams, setOptimizationParams] = useState<OptimizationParams>({
    min_discount: 5, max_discount: 50, target_revenue: 10000, risk_tolerance: 0.3, seasonal_weight: 0.4, segment_weight: 0.6
  });;
...
const response = await fetch('/api/ai/voucher/optimize', {
        method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(optimizationParams)
      });
...
type="number"
                  ...
                  onChange={(e) => setOptimizationParams({
                    ...optimizationParams, min_discount: parseFloat(e.target.value)
                  })}
...
type="number"
                  ...
                  onChange={(e) => setOptimizationParams({
                    ...optimizationParams, max_discount: parseFloat(e.target.value)
                  })}
                  inputProps={{ min: 0, max: 100 }}
                  fullWidth
                />
...
type="number"
                  ...
                  onChange={(e) => setOptimizationParams({
                    ...optimizationParams, target_revenue: parseFloat(e.target.value)
                  })}
                  fullWidth
                />
              </Grid>
              <Grid item xs={12,} sm={6,}>
                ...
                <Slider
                  ...
                  onChange={(_, value) => setOptimizationParams({
                    ...optimizationParams, risk_tolerance: value as number
                  })}
                  min={0,}
                  max={1,}
                  step={0.1,}
              ...
              <Grid item xs={12,} sm={6,}>
                ...
                <Slider
                  ...
                  onChange={(_, value) => setOptimizationParams({
                    ...optimizationParams, seasonal_weight: value as number
                  })}
                  min={0,}
                  max={1,}
                  step={0.1,}
              ...
              <Grid item xs={12,} sm={6,}>
                ...
                <Slider
                  ...
                  onChange={(_, value) => setOptimizationParams({
                    ...optimizationParams, segment_weight: value as number
                  })}
                  min={0,}
                  max={1,}
                  step={0.1,}
...
variant="contained"
      ...
      <Dialog open={analyticsDialog,} onClose={() => setAnalyticsDialog(false),} maxWidth="lg" fullWidth>
        ...
        <DialogContent>
          {stats && (<div className="space-y-6 mt-4">, <Grid container spacing={3, }>
