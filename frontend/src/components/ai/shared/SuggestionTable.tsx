...
export function SuggestionTable<T extends { id: string }>({
  data, columns, onView, onOptimize, loading = false, emptyText = 'Keine Vorschläge gefunden', className = '', }: SuggestionTableProps<T>) {
  return (<TableContainer component={Paper, } className={className, }>
      <Table size="small">
        ...
        <TableBody>
          {data.length === 0 && (<TableRow>, <TableCell colSpan={columns.length + ((onView || onOptimize) ? 1 : 0)} align="center">
                {emptyText,}
              </TableCell>
            </TableRow>
          )}
          {data.map((row) => (<TableRow key={row.id, } hover>
              ...
              {(onView || onOptimize) && (<TableCell align="center">, <div className="flex gap-1 justify-center">, {onView && (, <Tooltip title="Details anzeigen">, <IconButton, size="small", aria-label="Details anzeigen", onClick={() => onView(row),}
                        >
                          <ViewIcon />
                        </IconButton>
                      </Tooltip>
                    )}
                    {onOptimize && (<Tooltip title="Optimieren">, <IconButton, size="small", aria-label="Optimieren", onClick={() => onOptimize(row),}
                        >
                          <SettingsIcon />
                        </IconButton>
                      </Tooltip>
