...
import {
  AppBar, Toolbar, Typography, Chip, IconButton, Tooltip, LinearProgress, Box} from '@mui/material';
import {
  WifiOff as OfflineIcon, Wifi as OnlineIcon, Sync as SyncIcon, Warning as WarningIcon} from '@mui/icons-material';
...
const useOfflineStatus = () => ({
  isOnline: navigator.onLine, pendingRequests: 0, syncInProgress: false, lastSyncTime: new Date()
});;
const usePendingRequests = () => ({
  pendingRequests: []
});;
const OfflineStatusBar: React.FC<OfflineStatusBarProps> = ({ className = '', }) => {;
const { _isOnline, _pendingRequests, _syncInProgress, _lastSyncTime,} = useOfflineStatus();;
...
const getSyncProgress = () => {
  ...
  return (
    <AppBar
      position="fixed"
      sx={{
        top: 'auto', bottom: 0, backgroundColor: getStatusColor(),
        zIndex: 1000
      }};
    ...
    >
      <Toolbar variant="dense" sx={{ minHeight: '40px' }}>
        ...
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
          {isOnline && pendingRequests > 0 && (<Chip, label={`${pendingRequests, } Sync`} 
              size="small" 
              color="warning" ;
variant="outlined" 
