...
import { 
  Card, CardContent, Typography, TextField, FormControl, InputLabel, Select, MenuItem, Checkbox, FormControlLabel, Button, Box, Chip, Alert, CircularProgress, Divider} from '../../utils/muiImports';
...
export const NachrichtenFormular: React.FC<NachrichtenFormularProps> = ({
  onMessageSent, initialData, showKIVorschlaege = true, }) => {;
const [formData, setFormData] = useState<NachrichtenFormData>({
    empfaengerGruppe: initialData?.empfaengerGruppe || 'allgemein', betreff: initialData?.betreff || '', inhalt: initialData?.inhalt || '', leseBestaetigungErforderlich: initialData?.leseBestaetigungErforderlich || false, archivierungErzwingen: initialData?.archivierungErzwingen || false, autoProtokollAnhaengen: initialData?.autoProtokollAnhaengen || false, prioritaet: 'normal', kategorie: 'allgemein', tags: []
  });;
...
const gesendeteNachricht = await messagingService.createMessage(nachricht);
      ...
      setFormData({
        empfaengerGruppe: 'allgemein', betreff: '', inhalt: '', leseBestaetigungErforderlich: false, archivierungErzwingen: false, autoProtokollAnhaengen: false, prioritaet: 'normal', kategorie: 'allgemein', tags: []
      });
...
const handleKIVorschlag = (vorschlag: KINachrichtenVorschlag) => {
    setFormData(prev => ({
      ...prev, betreff: vorschlag.betreff, inhalt: vorschlag.inhalt, empfaengerGruppe: vorschlag.empfaengerGruppe, prioritaet: vorschlag.prioritaet
    }));
  ...
  return (
    <Card className="max-w-4xl mx-auto">
      <CardContent className="p-6">
        ...
        {successMessage && (, <Alert severity="success" className="mb-4">, {successMessage, }
          </Alert>)}
        ...
        <form onSubmit={handleSubmit,} className="space-y-6">
          ...
          <FormControl fullWidth error={!!errors.empfaengerGruppe,}>
            ...
            <Select
              ...
              onChange={(e) => setFormData(prev => ({ 
                ...prev, empfaengerGruppe: e.target.value as EmpfaengerGruppe 
              }))}
              label="Empfängergruppe"
            >
              {empfaengerGruppen.map(gruppe => (, <MenuItem key={gruppe.value, } value={gruppe.value, }>
                  <Box>
          ...
          <Box display="flex" gap={2,}>
            <FormControl fullWidth>
              ...
              <Select
                ...
                onChange={(e) => setFormData(prev => ({ 
                  ...prev, prioritaet: e.target.value as any 
                }))}
                label="Priorität"
              >
                {prioritaeten.map(prioritaet => (, <MenuItem key={prioritaet.value, } value={prioritaet.value, }>
                    <Chip 
            ...
            <FormControl fullWidth>
              ...
              <Select
                ...
                onChange={(e) => setFormData(prev => ({ 
                  ...prev, kategorie: e.target.value as any 
                }))}
                label="Kategorie"
              >
                {kategorien.map(kategorie => (, <MenuItem key={kategorie.value, } value={kategorie.value, }>
                    {kategorie.label, }
                  </MenuItem>))}
              </Select>
            </FormControl>
            ...
            <FormControlLabel
              control={
                <Checkbox,
                  ...
                  onChange={(e) => setFormData(prev => ({ 
                    ...prev, leseBestaetigungErforderlich: e.target.checked 
                  }))}
                />
              }
            ...
            <FormControlLabel
              control={
                <Checkbox,
                  ...
                  onChange={(e) => setFormData(prev => ({ 
                    ...prev, archivierungErzwingen: e.target.checked 
                  }))}
                />
              }
            ...
            <FormControlLabel
              control={
                <Checkbox,
                  ...
                  onChange={(e) => setFormData(prev => ({ 
                    ...prev, autoProtokollAnhaengen: e.target.checked 
                  }))}
                />
              }
          ...
          {/* KI-Vorschläge */,}
          {showKIVorschlaege && kiVorschlaege.length > 0 && (<Box>, <Button, variant="outlined", onClick={() => setShowVorschlaege(!showVorschlaege),};
className="mb-2"
              >
                🤖 KI-Vorschläge anzeigen ({kiVorschlaege.length, })
              </Button>

              {showVorschlaege && (<Card variant="outlined" className="mt-2">, <CardContent>, <Typography variant="h6" gutterBottom>, KI-Vorschläge, </Typography>, <Box className="space-y-2">, {kiVorschlaege.map((vorschlag, index) => (<Box key={index, } className="p-3 border rounded-lg hover:bg-gray-50 cursor-pointer" onClick={() => handleKIVorschlag(vorschlag),}>
