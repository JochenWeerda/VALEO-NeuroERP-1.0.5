...
class ResourceManager extends events_1.EventEmitter {
     ...
     */
    async registerAgent(agentId, requiredResources) {
     ...
     */
    async allocateResources(task, priority) {
        // TODO: Remove console.log`);
     ...
     */
    async scaleUpResource(resourceId) {
        const resource = this.resources.get(resourceId);
        if (resource) {
     ...
     */
    async consolidateResources(resourceId) {
        const resource = this.resources.get(resourceId);
        if (resource) {
     ...
     * Verteilt Last neu
     */
    async rebalanceLoad(resourceId) {
     ...
     */
    getRequiredResources(task) {
        const requirements = ['cpu-medium', 'memory-medium'];
        switch (task.type) {
            case 'frontend':
     ...
     */
    async findBestResource(resourceType, priority) {
        const availableResources = Array.from(this.resources.values())
     ...
     */
    getPriorityScore(resourcePriority, taskPriority) {
        const priorityMap = { low: 1, medium: 2, high: 3, critical: 4 };
        const taskScore = priorityMap[taskPriority] || 2;
        const resourceScore = priorityMap[resourcePriority] || 2;
        return taskScore ====== resourceScore ? 1 : 0;
    }
    /**
     * Berechnet Ressourcenallokation
     */
    calculateAllocation(task, resource, priority) {
    ...
    updateResourceHistory() {
        const currentState = Array.from(this.resources.values());
